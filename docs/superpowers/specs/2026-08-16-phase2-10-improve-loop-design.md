...
3. 従量課金経路が無い (env に鍵が無い — §1.1)。**どの子プロセス (trade worker を含む) の初期 env にも秘密を置かない** — trade worker の資格情報は handshake で渡す (R10-①)
...
- **起動契機と週期の CAS (codex 3 周目 I11)**: `schedule.improve` (`weekly` / `daily`) と **`schedule.improve_at`** (`"<weekday> HH:MM"` = weekly / `"HH:MM"` = daily、表示 TZ `display_timezone`。既定 `"Sat 03:00"`)。scheduler tick は **period key** (weekly = ISO 週 `YYYY-Www`、daily = `YYYY-MM-DD`、表示 TZ で計算) を求め、**その period の `improve_at` 時刻以後の最初の tick** で `INSERT OR IGNORE INTO improve_waves(period_key, created_at, m) VALUES (?, ?, ?)` を実行する — **`rowcount=1` のときだけ wave を起こす** (1 period 1 wave の CAS。再起動・多重 tick で二重にならない。**時刻を過ぎていれば catch-up** する)。新テーブル `improve_waves(period_key TEXT PRIMARY KEY, created_at TEXT NOT NULL, m INTEGER NOT NULL)`。**M=0 (空きスロットなし) は period を消費しない** (INSERT しない → 次 tick で再試行)。**部分 submit は period を消費する** (未 submit の partition は activity に記録し、次 period へ)。**手動**: 対話シェル `improve` (M=1 の wave、**分担なし** = 全バックログを見る、period を消費しない)。どちらも missions 行 `loop='improve'`, `trigger` は NULL (設計書 §12 — trigger は trade 専用)
...
- **wave** (codex 1 周目 I3): ①`M = min(improve.parallel, 空きスロット数)` を原子的に決める ②`open|observation` の backlog id を**不変スナップショット**として取る ③partition `k=0..M-1` を一度ずつ予約し ④M 件の Mission を `k` 付きで submit する。分担は `id % M == k`。**親が担当集合 `allowed_backlog_ids` を RunContext に固定し、出力の既存 `selected.backlog_id` が集合外なら Tx-1 より前に Mission failed** (codex 3 周目 I10 — prompt の規律だけに頼らない)。**新規 idea は例外** (集合に無くてよい)。**手動 M=1 は全 id が許可集合**。発見・リサーチで新規に見つけた課題は自由に追加してよい
...
- **SQLite 接続は slot (Mission) ごとに専用** (codex 3 周目 I12): 各 slot スレッドが prepare で `db.connect()` して所有し、`finally` で close する。**スレッド間で接続を共有しない**。競合の直列化は Tx-1/Tx-2 (§4.1) と `busy_timeout` にだけ任せる。取引レーンの接続とも共有しない
- **重複解決の線形化点は §4.1 Tx-1 の backlog CAS** (先に `selected` を取った方が勝ち、後着は observation)
- **shutdown / join** (I4/I9): `ImproveSupervisor.shutdown()` は `stop_event` を立てて新規 submit を拒否し、走行中の N 本の WorkerRunner に stop を伝播 (`WorkerRunner` の既存 `stop_event` を共有 — 各 WorkerRunner が自分の worker を kill し、**`cli_started` で受け取った CLI pgid も killpg** してから `MissionResult` を返す) → 各 commit 相の `finally` が missions 行を終端 → `ImproveSupervisor.join(shutdown_join_timeout_sec)`。`run_service` の停止シーケンスは取引レーンの `supervisor.join` と**同じ位置で**改善レーンも join する (取引側の順序は変えない)。**worker が EOF/異常死したときも** WorkerRunner は finalize 前に CLI pgid を killpg する
...
| 参照 | サンプル plugin の場所 (`docs/examples/plugins/`)・plugin 契約の要約・候補置き場のパス (`staging_dir`)・承認済み plugin の読取専用スナップショットのパス (`source_snapshot_dir`)・**plugin 名の正規形**・担当 backlog id 集合 | RunContext |
...
| `read_plugin_source(name)` | worker | **親が prepare で作った読取専用スナップショット `workdir/source/<name>/`** (承認済み live の 3 ファイルのコピー) を読む (改良の起点)。worker は `plugins/` 本体を見ない (codex 3 周目 I6) | 読取のみ |
...
timeout / 出力不正 / worker 異常死 → missions 行を該当 status で終端、`improvement_runs` は `result=NULL` のまま `finished_at` を書く (新しい終端値は足さない — 既存 CHECK `('approval','report')` を維持し、失敗は missions 側で読む)、staging を削除、**RPC 台帳を `DISCARDED` (`backtest_runs` / `analysis_runs` は書かれない)**。**再試行はしない** (次の wave で自然に再実行。R8 により課題は消えない)。**プロセス crash (Tx-1 後) の回収**: 起動時の `missions.recover_interrupted` と同じ transaction で、interrupted になる improve Mission に結びついた run (`improvement_runs.mission_id`) の backlog `selected` 行を `observation` (`last_result='interrupted'`) へ戻す (§4.1 Tx-1 が run↔backlog↔mission を durable に結ぶ — codex 3 周目 I2)。
...
`ImproveLoop` は `TradeLoop` と同じ prepare / run / commit の三相。**prepare** (core_lock 内・短時間、slot スレッド・専用接続): missions 行作成 → `improvement_runs.start` は Tx-1 へ (下記) → workdir/staging (0700)/`source/` スナップショット (承認済み live の 3 本を 0500 でコピー)/認証コピー (§1.1-2) → 注入コンテキスト生成 → **`ImproveRunContext` を構築** (immutable: `mission_id` / `staging_dir` / `source_snapshot_dir` / `allowed_backlog_ids` / `ledger` (`ImproveRpcLedger`, OPEN) / `rpc_handlers` (`run_backtest` / `analyze_corr` の親側実装、専用接続に束縛)) → その Mission 専用の `WorkerRunner(..., worker_profile="improve", run_context=ctx)` を構築 (codex 3 周目 I5。`Mission` は不変。handshake は ctx から `mission_id` / `staging_dir` / `source_snapshot_dir` を別フィールドで運び、子が相互照合する)。**run** (lock 外): `WorkerRunner.run(mission)`。**commit** (lock 外、専用接続、`missions.finish` の CAS で終端)。

### 4.1 transaction 設計 (codex 2 周目 C2 / 3 周目 I2・I3・I12) — 長い仕事は transaction の外、DB 書込は短い 2 回

SQLite の writer は 1 本なので、**`BEGIN IMMEDIATE` を pytest / バックテスト越しに保持してはならない** (取引レーンが busy timeout を踏み R7 が破れる)。接続は Mission ごとに専用 (§3.1)。commit 相の DB アクセスは次の 2 つの短い transaction に閉じる:

- **Tx-1 (早い短い tx、選択の線形化 + 所有の永続化)**: `BEGIN IMMEDIATE;` ①`discoveries` の INSERT (重複・上限は §4.2-2) ②`UPDATE improvement_backlog SET status='selected', attempts=attempts+1, updated_at=? WHERE id=? AND status IN ('open','observation')` — `rowcount=1` がこの Mission を唯一の勝者にする (I2) ③**`improvement_runs.start(backlog_id, mission_id, now)`** — `improvement_runs` に **`mission_id INTEGER` 列を `ensure_column` で追加**し、run ↔ backlog ↔ mission を durable に結ぶ (codex 3 周目 I2。これが無いと Tx-1 直後の crash で `selected` が永久に不可視になる) → `COMMIT`。新規 idea なら同じ tx で INSERT → UPDATE。`rowcount=0` (並行 Mission が先に取った / 人間が閉じた) → **敗者経路**: run は `backlog_id=NULL` で start、ゲート作業を一切せず、レポート (「重複のため見送り」) と `improvement_runs.finish` だけを Tx-2 で書く
...
- **Tx-2 (最後の短い tx)**: `BEGIN IMMEDIATE` → 台帳の `backtest_runs` / `analysis_runs` 行 → 親ゲートの `backtest_runs` 行 (in-sample / holdout_gate) → `approvals.create(commit=False)` → backlog 遷移 (§4.3) + `last_result` → `improvement_runs.finish(commit=False)` → `COMMIT`。**その後**に `missions.finish` の CAS。途中失敗は全部ロールバック (staging は残り、Mission は failed) → 別の短い tx で backlog を `observation` (`last_result='commit_failed'`) へ
- **store helper に caller-owned transaction 版を用意する**: `save_harness_run` / `approvals.create` / **`approvals.decide`** / **`approvals.expire_due` (行を 1 件ずつ列挙して処理する版)** / `improve_runs.start` / `improve_runs.finish` / `analysis_runs.save` / `backlog.set_status` の各々に `commit=False` (呼び出し側が transaction を持つ) 変種。現行の内部 `conn.commit()` はそのまま残し (他の呼び出し元は不変)、improve レーンと承認決定 (§4.3 の `apply_decision`) だけが変種を使う
- **起動時回収 (I2)**: `missions.recover_interrupted` と同じ transaction で、interrupted にする improve Mission を `improvement_runs.mission_id` で引き、その run の backlog が `selected` なら `observation` (`last_result='interrupted'`)、run は `finished_at` を埋め `result=NULL`
...
2. **バックログ追記 + 選択 + 所有 (Tx-1)**: `discoveries` を追加 (`source` = agent|research)。**正規化 (空白・大小文字) した `idea` の完全一致は重複として捨てる**。**上限 `improve.max_new_backlog_per_mission` (既定 20)** — 超過分は捨てて activity に件数を記録。続けて選択の CAS と `improvement_runs.start` (§4.1)。敗者 → 敗者経路
...
9. **`COMMIT` → `missions.finish` CAS → 掃除**: 承認申請を出した staging は残す。それ以外は削除。台帳 `PERSISTED`。接続 close
...
| 現在 | 事象 | 次 | `last_result` |
|---|---|---|---|
| open / observation | Mission が選択 (Tx-1 CAS、`attempts+1`、run を `mission_id` 付きで start) | selected | (変更なし) |
...
2. **後発決定の確認 (ⓓ)**: 同一 plugin 名へのより新しい決定 (reject) があれば、この承認は失効 — `apply_decision(status='invalidated')` (§4.3。backlog も同 tx で `observation`)。再試行で復活させない (D4 の順序規則)
...
5. **旧プレーン版の保全 (初回移行のときだけ、codex 3 周目 I1)**: `plugins/<name>` が**プレーンなディレクトリ**なら、lock 下でその 3 本をスナップショット検査 → `artifact_hash_old` を計算 → **`plugins/.versions/<name>/<artifact_hash_old>/` へコピー** (4 と同じ tmp → fsync → rename、冪等) → **bare 履歴へ「adopted pre-existing」として記録** (5.2 の plumbing、commit message に `adopted pre-existing content=<…> artifact=<artifact_hash_old>`。既に同 tree なら no-op)。これで旧版も版ストアと履歴に残り、`plugin rollback` で戻れる
...
8. **`apply_decision(status="approved")`** (§4.3 — approval 行 CAS + backlog `done` を 1 tx で。決定は最後)。ここで初めて次回起動の `approved_plugins()` に載る
9. staging を削除、lock 解放

**却下・期限切れ・失効**: `apply_decision` (backlog を `observation`) + staging 削除。`plugins/<name>`・版・履歴には触れない。
...
5. **改善レーンが取引レーンを塞がない**: 改善 Mission 実行中に `MissionSupervisor.try_submit("trade")` が受理される / 取引レーンの容量 1・直列性の既存 pin が不変 / **wave が `parallel=N` で N partition を全て担当する** / **backlog 選択の CAS** (2 接続同時で勝者 1) / **backlog 状態機械** (approve → done、reject/expire → observation、report → done / report_failed → observation、失敗 → observation、**crash 後の起動時回収 → observation:interrupted**) / **台帳の凍結** (FROZEN 後の遅延 RPC が拒否され、payload `trial_count` が実 trial の総和) / **担当外 backlog_id を親が拒否** / **1 period 1 wave** (`improve_waves` CAS、catch-up、M=0 は period 非消費、再起動で二重 wave 無し) / **接続は slot 専用** (N=4 同時 Tx で混線無し) / shutdown が改善レーンの worker と CLI pgid を全て回収する / **認証コピーは親が spawn 前に行い、worker は原本を読まない**
...
| C | 8 | バックログ拡張 (`observation` / attempts / last_result / **選択 CAS / 状態機械ヘルパ `apply_approval_outcome` / `approvals.apply_decision`** / `backlog reject\|reopen`) + `improvement_runs.mission_id` + `improve_waves` テーブル + 起動時回収 (interrupted → observation) + store helper の `commit=False` 変種 (`decide` / `expire_due` 含む) + 注入コンテキスト生成 + prompt | §3.1/§3.2/§4.1/§4.3 | — |
| **D** (loop) | 9 | `ImproveSupervisor` (N スロット・wave・slot 専用接続・shutdown/join + CLI pgid 回収) + scheduler の period key / `improve_waves` CAS / catch-up + `improve` / `improve add` / `backlog` / `policy add` コマンド (**`improve` の有効化配線は Task 12**) | §3.1 | 8 |
| D | 10 | `ImproveLoop` (三相 + `ImproveRunContext` + source snapshot + Tx-1 (CAS + run start) / Tx-2 + commit 相ゲート + 担当集合検査 + 台帳永続化 + 承認申請 + レポート (O_EXCL\|O_NOFOLLOW、失敗時 result=NULL + report_failed) + backlog 遷移) | §4 | 7, 8, 6 |
| **E** (承認) | 11 | 版ディレクトリ (`artifact_hash`) + bare 履歴 (blob-level plumbing) + **旧プレーン版の adopt** → symlink 切替 (rename / `RENAME_EXCHANGE`) + `flock` + reconcile (孤児 staging・tmp 版・未参照版・temp link・旧 dir 残骸の adopt 再開・dangling) + bless 経路 (初回 symlink 化) + **`approval retry` / `plugin rollback` の handler と配線** + 全決定経路の `apply_decision` 化 | §5 | 6, 8 |
