...
- 日付: 2026-08-16 (初版)
- ステータス: **収束 (2026-08-17)**。ユーザー承認済み (2026-08-16) + codex 設計レビュー **14 周**を反映 (10 周目後に R12 でスコープ縮小)。**14 周目の判定: Critical 0 / Important 0 / Minor 2 (適用済み) — 「設計として実装計画へ渡せる状態である」**。次: 実装計画 (`superpowers:writing-plans`)。入力は §8 (task 表) と §8.1 (実装計画へ送る項目・確定版)
...
| D6 の到達範囲 (R12) | 「plugin 履歴とロールバック」 | **本プランは履歴 (bare git) と版ストアの保管まで**。ロールバック操作・旧版 adopt・**既存プレーン plugin の自動移行** は起票 (§A) — 戻すときは `materialize` → 編集 → `bless --from _human` で新規承認、プレーン live は人間の `plugin retire` で退避 |
...
- **起動時検査は撤去 (2026-09-12 test-hygiene T4)**: 旧稿は provider `chatgpt` のとき `auth.json` の `chatgpt_subscription_active_until` を起動時検査で読み、期限が近い/過ぎていれば警告するとしていたが、実 `~/.codex/auth.json` (codex CLI 0.150.1) にこのキーは存在しない (4 run 連続で「キーが無い」WARNING が空振り) ため撤去した。期限切れ・利用上限は codex 自身が実行時に返す stderr 文言 (`usage limit` / `try again at` / `401 Unauthorized`) を `cli_runner.CLI_STDERR_FATAL_PATTERNS` (`cli_stderr_fatal`) で検知する — 起動時検査ではなく実行時検知。**local への自動フォールバックはしない** (設計書 §13 の既存裁定と同じ)
...
- **起動時検査** (`service.build_app` の既存 `_check_llama_swap` の隣): 選択された backend について ①`bin` を `shutil.which` → `realpath` で**絶対パスへ正規化**し、実行可能な通常ファイルであること。**codex は ELF (vendor native) を要求し、node ラッパ (`codex.js` / shebang スクリプト) は拒否** — エラー文言に探し方 (`npm root -g` 配下 `@openai/codex/node_modules/@openai/codex-linux-x64/vendor/.../bin/codex`、または `codex --version` を打つラッパの中身) を書く (codex 3 周目 I8。node ラッパの closure は起票 §10) ②`<bin> --version` が 0 で返る (rlimit なし・親プロセス側で) ③認証ファイルが存在する (**claude と codex+chatgpt のみ**。codex+llama_swap は要求しない — §1.1-2) **④ (codex+chatgpt の `chatgpt_subscription_active_until` 起動時検査) は撤去 (2026-09-12 test-hygiene T4)** — 実 `~/.codex/auth.json` にこのキーが無く空振り WARNING しか出なかったため。期限切れ・利用上限は codex 自身の実行時 stderr (`usage limit` / `try again at` / `401 Unauthorized`) を `cli_stderr_fatal` で検知する側に寄せた ⑤**improve+claude のとき、サービス自身の初期 env (`/proc/self/environ`) に秘密名パターン (`*_API_KEY` / `*TOKEN*` / `*SECRET*` / `*WEBHOOK*` / `ANTHROPIC_*` / `OPENAI_*`) の変数があれば起動拒否** — 理由: claude worker は `/proc` を読めるため親の初期 env を読み戻せる (R10)。文言は「秘密は exported env でなく `.env` に置くこと」(`python-dotenv` は `os.environ` に setenv するだけで `/proc/self/environ` には現れない)。**①②③⑤のいずれかを欠けば `build_app` は起動拒否 (fail closed)** — Landlock 不可時の improve 起動拒否と同じ扱い。backend=local の環境では一切走らない
...
- **wave と slot (codex 4 周目 I4 / 5 周目 I1 / 6 周目 I1 / 7 周目 I1・I2、R12 で再開機構を簡素化)**: 新テーブル `improve_waves(period_key TEXT PRIMARY KEY, created_at TEXT NOT NULL, expected INTEGER NOT NULL)` と **`improve_wave_slots(wave_period_key TEXT NOT NULL REFERENCES improve_waves(period_key), k INTEGER NOT NULL, status TEXT NOT NULL CHECK(status IN ('reserved','claimed','running','done','failed')), mission_id INTEGER REFERENCES missions(id) (裁定 D1 2026-08-24), spawn_attempts INTEGER NOT NULL DEFAULT 0, created_at TEXT NOT NULL, updated_at TEXT NOT NULL, PRIMARY KEY(wave_period_key, k))`**。同じ裁定 D1 (2026-08-24) で `improvement_runs.mission_id`（§4.1、`_ensure_column` 由来）にも `REFERENCES missions(id)` を追加する（既存 DB は table rebuild で移行、`store/db.py` の `_migrate_improvement_runs_mission_id_fk` / `_migrate_improve_wave_slots_mission_id_fk`）。**slot は帳簿であって再開の単位ではない** (R12-(c))。手順: ①`M = min(improve.parallel, 空きスロット数)`。**M=0 なら何も書かない** (period 非消費、次 tick で再試行) ②**1 つの短い tx** で `INSERT OR IGNORE INTO improve_waves(period_key, created_at, expected) VALUES (?, ?, M)` (`rowcount=1` = 起動権) + **同じ tx で slot 行 `k=0..M-1` を `reserved` で INSERT** — **wave 行が存在した時点で period は消費済み** (実行 0 件でも次 tick で再 CAS しない。取りこぼした period は人間の `improve` で補う) ③**3-way 起動 (7 周目 I1)**: `reserved → claimed` (§4.1 Tx-0 と同じ tx で CAS `UPDATE … SET status='claimed', mission_id=?, spawn_attempts=spawn_attempts+1 WHERE … AND status='reserved'`) → worker spawn → worker `ready` → **親が短い tx で `claimed→running` を commit** → **親が `go` フレームを送る** → worker が Mission を開始 (`go` 前は agent/ツール実行ゼロ。`go` が来なければ副作用ゼロで終了)。`running` の commit と `go` の間で親が落ちても、worker は `go` を待って終了し、slot は `running` のまま → 起動時回収で `failed` (再実行しない) — 「実行済み Mission の再実行」は起きない ④**分担は負荷分散のヒント**: 親は prepare で**そのときの** `open|observation` 集合から `id % M == k` で `allowed_backlog_ids` を計算し RunContext に入れる (**永続化しない** — codex 7 周目 I3、簡素化側)。正しさは §4.1 Tx-1 の CAS が担う (敗者 → observation)。**手動 M=1 は全 id** ⑤**終端の直積 (7 周目 I2)** — 下表 ⑥**pre-ready の失敗** (spawn 失敗 / `ready` 前 crash・timeout) は**同一プロセス内でのみ** slot を `reserved` に戻して再 claim、**`spawn_attempts` は claim ごとに +1、失敗時に `< 2` なら `reserved` (初回 + 再試行 1 回)、それ以外は `failed`**。**`reserved` に戻すときは同じ tx で `mission_id=NULL` も戻す** (codex 12 周目 I3 — 戻した直後に crash すると ⑦ の起動時回収が assert する `mission_id IS NULL` を満たせず、slot が永久に非終端になる) ⑦**再起動後は再開しない**: `status IN ('claimed','running')` の slot で mission が interrupted になるもの、および `reserved` のまま残った slot (`mission_id IS NULL`) は全て `failed` (period は消費済みのまま)。**`started_at`・「全 failed の wave を削除して period を返す」規則は持たない** (簡素化) ⑧`submitted` は列で持たず `status IN ('running','done','failed')` の slot 数から導出 ⑨**手動 `improve` は wave/slot 行を作らない**
  ...
  | 起動時: 既存 wave の `reserved` slot (`mission_id IS NULL` を assert — wave 作成 COMMIT 後・最初の Tx-0 前の crash) | → failed (wave 行は残す = period 消費済み) | — | — | `recover_interrupted` と同一 tx (codex 11 周目 I5) |
...
RPC は既存 `tool_rpc` フレーム (in-flight 1) に `run_backtest` / `analyze_corr` を追加する。**親側 dispatcher スレッドは自前の読取専用接続 (`connect_readonly`、Mission 専用) で handler を実行する** (§3.1 接続の所有)。dispatcher は `rpc_timeout_sec` を **RPC 種別ごと**に持つ (バックテストは数十秒〜数分。`improve.backtest_rpc_timeout_sec` 既定 600)。**RPC 台帳** (`ImproveRpcLedger`、Mission ごと・メモリ、lock 付き状態機械 `OPEN → FROZEN → PERSISTED | PERSIST_FAILED | DISCARDED`): dispatcher は handler 起動前に受理予約を取り、**期限内に結果を受理した直後**に各呼出しの `{opaque_ref, kind, params, result_summary, trial_count}` を記録して予約を解放する。handler 自身は記録しない。timeout / handler error は予約だけを解放し、結果を記録しない。`trial_count` は `analysis_runs.trial_count` と同じ意味 (= 計算した相関値の個数。lead-lag 1 呼出しでも 25 になり得る) で、`analyze_for_agent` が返す値をそのまま持つ。commit 相は先頭で台帳を**原子的に `FROZEN`** にしてから読む。**FROZEN 以後に届く遅延 RPC 完了は missions.status に関係なく拒否** (activity にカウント)。**RPC timeout (`rpc_timeout_sec` 超過) した呼出しは監査上「試行」に数えない** — その遅延結果は捨てる。監査に載るのは dispatcher が期限内に受理した分のみ (この規則を payload の説明文にも焼く)。台帳は Mission の全終端で `mission_outcome` 付きで永続化し、外側 commit 成功後に `PERSISTED`、保存の一時失敗は再試行可能な `PERSIST_FAILED`、予約打ち切りや CAS 負けなど再試行しない場合だけ `DISCARDED` とする。
...
timeout / 出力不正 / worker 異常死 → **`finish_improve_mission` (§4.1) の 1 tx** で missions 行を該当 status で終端、**prepare で作った `improvement_runs` の行** (§4.1 run lifecycle) を `result=NULL` のまま `finished_at` で終端、scheduler wave なら slot を `failed` (新しい終端値は足さない — 既存 CHECK `('approval','report')` を維持し、失敗は missions 側で読む)、staging を削除する。**RPC 台帳は成功・失敗を問わず全終端で `mission_outcome` 付きの `backtest_runs` / `analysis_runs` として同じ終端 transaction 内に永続化する**。保存失敗は終端処理を妨げず `PERSIST_FAILED` とし、補償処理から再試行できる。期限内受理の監査境界は dispatcher が所有するため、timeout 後に完了した handler の結果は永続化対象にならない。**プロセス crash (Tx-1 後) の回収**: 起動時の `missions.recover_interrupted` と同じ transaction で、interrupted になる improve Mission に結びついた run (`improvement_runs.mission_id`) の backlog `selected` 行を `observation` (`last_result='interrupted'`) へ戻す (§4.1 Tx-1 が run↔backlog↔mission を durable に結ぶ — codex 3 周目 I2)。
...
- **Tx-2 (最後の短い tx — 終端の単一 commit point、codex 4 周目 I1)**: `BEGIN IMMEDIATE` → **全終端で** dispatcher が期限内受理した台帳の `backtest_runs` / `analysis_runs` 行 (`backtest_runs.mission_outcome` に当該終端結果) → 親ゲートの `backtest_runs` 行 (in-sample / holdout_gate) → `approvals.create(commit=False)` → **`finish_improve_mission(..., mission_status='completed', slot_status='done', run_result=…, backlog_transition=…, commit=False)` (単一ヘルパ — 中で backlog 遷移 + `last_result`、`improvement_runs.finish`、`missions.finish` の CAS `WHERE id=? AND status='running'` (影響行 0 なら例外 → 全体ロールバック)、slot `running→done` を行う。個別 UPDATE は書かない — codex 9 周目 M1)** → `COMMIT`。**approval・run・backlog・mission・台帳の終端が 1 つの commit point** — Tx-2 直後〜mission 終端の間の crash 窓は消える。台帳保存だけが失敗した場合は SAVEPOINT を巻き戻して終端を続行し、ledger を `PERSIST_FAILED` にして補償処理から再試行可能にする。途中失敗は全部ロールバック (staging は残る) → **補償 tx** (短い、**`finish_improve_mission`** 経由): 台帳を `mission_outcome='commit_failed'` で再保存してから、backlog を `observation` (`last_result='commit_failed'`)、run を `result=NULL, finished_at`、missions を `failed` (CAS)、**slot を `running→failed`** (scheduler wave のとき) で終端 (codex 8 周目 I2)。**`ready` 後の全終端 (Tx-2 成功・補償・出力検査不合格・timeout/failed/max_turns・shutdown) はこの 1 ヘルパだけを使う** — slot+mission+run+backlog を同一 tx で更新する唯一の経路
- **T3 是正 (test-hygiene 設計書 2026-09-12): `missions.finished_at` は論理終端時刻であり壁時計ではない。** `ImproveSupervisor` が worker の結果を受け取った直後に 1 回だけ束縛する `now` が、そのまま `_run_strategy_gate` (in-sample + holdout の複数 backtest、実測 ≒70 秒) を経由したあとの Tx-2 (`finish_improve_mission`) まで素通しされる — つまり `finished_at` は「決定論コアが親ゲートまで含めて確定した時刻」ではなく「worker プロセスが結果を返した時刻」であり、実際の所要時間より約 22% 過小に出る。実測が必要な場面 (診断・監視) は `activity` テーブルの該当行 (`approval_requested` / `gate_failed` 等、commit 成功後に書かれる) のタイムスタンプを使うこと。Clock 配線を変えて commit 直前に取り直す案も検討したが、精度改善に対して配線変更コストが見合わないため見送った (指揮者裁定 2026-09-12)。
...
- **T5 是正 (test-hygiene 設計書 2026-09-12): `plugins/_archive/INDEX.md` のヘッダ注記 (approval-quality v1.1 `7837fe9`) は新規作成時にしか書かれず、それより前から存在する既存ファイルには付かない。** 実害は人間が既存ファイルを読んだときヘッダが無く目録だと誤解しうる見た目の問題に留まるため、据え置き (現状維持) を既定とする — startup sweep への相乗り (T2 と同じ `sweep_orphans` で先頭書き換え) は安価だが、1 回限りの移行のために毎起動チェックが常時発生する形になるため見送った (指揮者裁定 2026-09-12)。
- **起動時回収 (I2/I1)**: `missions.recover_interrupted` と同じ transaction で、**`improvement_runs.finished_at IS NULL` かつその `mission_id` の Mission が interrupted になる run だけ**を対象に、run を `result=NULL, finished_at` で終端し、`backlog_id` が指す backlog が `selected` なら `observation` (`last_result='interrupted'`)、scheduler wave の slot は `status IN ('claimed','running') → failed`、加えて**既存 wave の `reserved` slot (`mission_id IS NULL`) も同 tx で `failed`** (= `finish_improve_mission` と同じ更新集合。§3.1 終端表、codex 11 周目 I5/M1)。**`finished_at` が入っているのに mission が `running` の run は Tx-2 の単一 commit point により存在しない** — これを起動時 assert (activity ERROR) で pin する
...
5. **承認申請 (Tx-2 の一部)**: `approvals.create(kind="plugin", commit=False, payload={name, kind, candidate_origin: 'staging', candidate_path: 'plugins/_staging/<mission_id>/<name>' (root 相対の正規形。`_human` 経路は 'plugins/_human/<name>' と `candidate_origin: 'human'`), content_hash: H_before, artifact_hash, mission_id, backlog_id, in_sample: {...}, holdout: {...}|null, baseline: {...}|null, analysis_run_ids: <台帳→Tx-2 で解決した実 id>, trial_count: <台帳 entries の trial_count の総和 = 計算した相関値の個数>, analysis_call_count: <台帳の analyze_corr 呼出数>, backtest_call_count: <台帳の run_backtest 呼出数>, selection_rationale: <agent>, summary, audit_note: "RPC timeout した呼出しは数えていない"})`。**id・回数は agent 出力から取らない** (I7)。**`bless` は経由しない**
6. **レポート (親だけが書く)**: `artifact.type=report`、およびゲート不合格・評価不能・observation・敗者のとき、`reports/` に書く。**`proposal_kind=risk_gate` の report は本プランでは受理しない — observation `unsupported_in_plan10` にして report ファイルは書かない** (R12-(b)。親側の評価設計 (intent 単位 key・affected pairs/strategies・baseline/候補の in-sample + holdout 比較) は §A に保存し §10 に起票。本体設計書 §6「risk gate 提案の根拠にも同じ規律」を満たせないため、根拠なしの report を出さない側に倒す)。`core` / `research` の report は評価を要さない。
   **書き方 (codex 5 周目 I6 / 6 周目 I2 — 失敗 Mission の部分/孤児 report を残さず、DB が存在しない report を指さない)**: 本文は tx 外で **親専有の一時領域 `data/improve_reports/.tmp/improve-<mission_id>.md.part`** (裁定 F-3 2026-08-27: 実装は `reports/` ではなく `data/improve_reports/` を outbox ディレクトリとして使っており、本節はこの実装値に統一する — 検収 acceptance-task12.md F-3) に `O_WRONLY|O_CREAT|O_EXCL|O_NOFOLLOW` (dirfd) で書き `fsync`。**Tx-2 は `improvement_runs.report_state='prepared'` + 予定の最終パス `report_path` を書く**。**最終名 `data/improve_reports/improve-YYYY-MM-DD-<mission_id>.md` (裁定 F-3 2026-08-27) への `rename` (公開、`RENAME_NOREPLACE`) は COMMIT の後、最後の一手** → **`data/improve_reports/.tmp` と `data/improve_reports/` の両ディレクトリを fsync** → 成功で短い tx `report_state='published'`。公開失敗 (最終名が既に在る・I/O) → **短い補償 tx** で `report_state='failed'`、`result=NULL`、`report_path=NULL`、backlog **`done → observation` (`report_failed:<reason>`、§4.3 — この遷移は report_state の失敗からのみ許す)**、activity + 通知。**Tx-2 が rollback したら `.part` を削除**。**起動時 reconcile は §4.1 「report の公開状態」の状態表のとおり** (ここでは再掲しない — codex 9 周目 M3)。**eventual な不変条件 (reconcile 後): `published ⇔ 最終ファイルが存在`、遷移中の `prepared+最終` は許容**。`data/improve_reports/` と `data/improve_reports/.tmp/` は起動時に親が mkdir、gitignore 済み、**worker の rw には無い** (§2.1)。**agent の `body_md` は「提案本文」節に引用として入れる — 信用しない**
7. **`improvement_runs.finish` (Tx-2)**: 承認申請を出したら `result='approval'` + `approval_id` (+ レポートの一時ファイルが書けていれば `report_state='prepared'` + 予定 `report_path`)。承認申請なしで**レポートの一時ファイル作成に成功したときだけ** `result='report'` + `report_state='prepared'` + 予定 `report_path`。一時ファイル作成に失敗していたら `result=NULL`, `report_state='failed'`。**公開 (rename) が COMMIT 後に失敗したら補償 tx で `report_state='failed'`, `result=NULL`, `report_path=NULL`** (codex 2 周目 M2 / 5 周目 I6 / 6 周目 I2 — DB が存在しない report を指さない)。
   **`improvement_runs.result` の値規約 (裁定 F-4 2026-08-27、検収 acceptance-task12.md F-4)**: `approval` = この Mission が承認申請を出して終端した (§4.3 状態表の「承認申請を発行」行に対応)。`report` = 承認申請を出さず、かつレポートの一時ファイル作成に成功した終端 — この規則は手順6 が列挙する**全ての**「承認申請を出さない report 経路」(ゲート不合格 / 評価不能 / observation / 敗者) に等しく適用される。**個別の終端メソッド (`_finalize_gate_failed`/`_finalize_loser`/`_prepare_report_if_applicable` 経由) ごとに別の値規約は無い** — 「report を書いたら `result='report'`、書けなければ `result=NULL`」の 1 本の規則がどの終端経路にも共通して効く。`NULL` = 承認申請も report も出さなかった終端 (`_finalize_failed_mission`/`_finalize_output_invalid` など、レポート一時ファイル作成に失敗した経路を含む)。
...
**`switched` で止まった未完ジャーナルの retry 処置 ([switch-ops-hardening] による具体化。上の「未完があれば先に reconcile でそれを完了/巻き戻してから決定する」規則自体は変えない)**: `phase='switched'` の行は、journal-first の順序 (先に `advance(switched)` を commit してから `switch_live` を呼ぶ) ゆえ live が実際に新版を指しているとは限らない。lock 内で live の指す先を分類し、新版を指していれば従来どおり再検証して決定するが (§5.1-1 の「完了」側)、**旧版のまま/live 不在 (`not_switched`) なら「巻き戻してから決定する」側**として扱う — ただし単に旧状態へ revert して pending に留めるのではなく、その停止行を巻き戻して終端させたうえで **新しい journal 行を作って手順を頭から流し、配備まで完了させる**。live が第三者に書き換えられている (`foreign`) ときだけは、どちらの側にも倒さず触らず人間待ちで止まる (fail closed)。
...
- **既存のプレーンな `plugins/<name>/` (手作り、本プラン以前)** は**自動では一切切り替えない** (codex 11 周目 I1/I2 — 稼働中に plain を入れ替えると `PluginMeta.path` が旧版を指し続けられず、復旧状態機械も plain 専用の phase を要する。R12 の精神でさらに簡素化)。discover は両形 (プレーン / symlink) を読み、両者は共存できる。**同名の候補が承認されたとき、live がプレーン dir なら**: 版ディレクトリの作成と git 記録までは行い (何も失われない)、**承認は理由 `legacy_plain_present` で pending に留める** (activity + 通知)。人間が **`afx plugin retire <name>`** (plugin `flock` 下。同名の未完ジャーナルがあれば拒否。プレーン dir を `plugins/_retired/<name>-<UTC タイムスタンプ>/` へ **1 rename** し activity `plugin_retired` を残す。**稼働中サービスは再起動まで discover 済みの旧 `PluginMeta.path` を使い続けるが、その path は `_retired` へ移動しているので次回起動まで実行が失敗しうる — retire は運用時間外に行う (運用ドキュメント)**) を実行し、続けて `approval retry <id>` を叩くと、切替は **absent → symlink の通常の rename** として完了する。`_retired/` は人間所有 (自動削除しない。discover は `_` 先頭を列挙しない)。**旧版の版化・履歴記録は行わない** — 人間が必要なら `_retired` から `_human` へコピーして `bless --from _human` で新規承認する。**自動移行 (`RENAME_EXCHANGE` による plain↔symlink の原子交換 + `retired_path` の巻き戻し) は起票** (§A.1 に設計あり)
- 履歴は **bare リポジトリ `plugins/.history.git`** (`git init --bare` を lazy に。**ワークツリーを持たない** — codex 2 周目 I9)。人間の閲覧は `git --git-dir=plugins/.history.git log|show`。**不変条件は一方向**: **`approved` ⇒ 版化済み かつ 履歴記録済み**。逆は保証しない — ゲートを通った pending の artifact (legacy plain 経路、記録直後に DB が落ちた中間状態) は版ストアと履歴に存在しうる (codex 13 周目 M2、§5.3)
...
3. **ハッシュ再照合 (ⓐ)**: payload の `candidate_origin` / `candidate_path` (root 相対の正規形。dirfd 基準で `plugins/_staging/<mission_id>/<name>` または `plugins/_human/<name>` の形だけを受理、`resolve()` は使わない) が指す候補を §4.2-3a と同じスナップショット検査に通し `content_hash` と `artifact_hash` を再計算、payload と一致しなければ承認は成立せず **pending のまま** + activity + 通知。**候補が無ければ `candidate_missing` で pending** (codex 11 周目 I4)。**候補 locator の所有と掃除**:

   | `candidate_origin` | `candidate_path` | 掃除 |
   |---|---|---|
   | `staging` (改善ループ) | `plugins/_staging/<mission_id>/<name>` | 終端決定 (approved / rejected / expired / invalidated) の tx 直後に削除。pending の間は残す (§2.3) |
   | `human` (`submit\|bless --from _human`) | `plugins/_human/<name>` | **自動では削除しない** (人間所有)。承認後も残る (人間が消す) |

4. **版ディレクトリの作成 (冪等)**: `plugins/.versions/<name>/<artifact_hash>.tmp-<op-identity>/` に 3 ファイルをコピー → 各ファイルとディレクトリを `fsync` → 0400/0500 に落として `rename` で `plugins/.versions/<name>/<artifact_hash>/` へ (既に同 artifact_hash の版があれば作らない)。失敗 → pending + 通知。**本番 symlink には触れていない**。**`<op-identity>` と phase 更新は経路で二分する (codex 13 周目 I1 — plain 経路にはジャーナルが無いので `op_id` を採番できない)**:

   | 経路 | `<op-identity>` | phase 更新 | crash 後 |
   |---|---|---|---|
   | live が **absent / symlink** (ジャーナル有り) | `<op_id>` (ジャーナル行 `preparing` を INSERT してから) | 完了後に `versioned` | ジャーナルの phase から再開 (§5.1-1 の収束規則) |
   | live が **プレーン dir** (ジャーナル無し、`legacy_plain_present` へ向かう) | **`approval-<approval_id>`** (operation identity は approval 行そのもの) | **一切更新しない** (phase 列を持たない) | **pending approval + 候補から頭から冪等に再実行**。lock 下で同名 temp が在れば hash を検証して再開、不一致なら削除してやり直す |

   **プレーン経路の temp 残骸の所有**: `<artifact_hash>.tmp-approval-<approval_id>` は**その approval が pending の間だけ有効**。approval が終端 (approved / rejected / expired / invalidated) したら起動時 reconcile が削除する (`GC_ROOTS` の対象外)
5. **履歴への記録 (ⓑ) — 新版ディレクトリの 3 本から、`<name>/<file>` のパスで** (ジャーナル有りの経路のみ完了後に `recorded`。**プレーン経路は phase を更新しない** — codex 13 周目 I1): 下記 5.2 の blob-level plumbing。失敗 (git 不在 / detached / identity 無し / CAS 上限) → **pending のまま + 通知。本番には一切触れていない**。**プレーン経路はここまで済んだら `legacy_plain_present` を理由に pending で終える** (人間の `retire` → `approval retry` が通常の P2 = ジャーナル `op_id` 有りの経路として最初からやり直す)
6. **切替 (原子操作 1 回、`switch_required=1` のときだけ)** (直前に switch ジャーナル `switched` を短い tx で書く — FS 効果より先): temp symlink `temp_path` = `plugins/.<name>.link-<op_id>` (→ `.versions/<name>/<artifact_hash>`) を作り、**`os.rename(temp, plugins/<name>)`** (不存在 → symlink / symlink → symlink の rename は原子的)。**`plugins/<name>` がプレーン dir ならここへ来ない** (手順 0 で `legacy_plain_present` pending に留まる。人間の `retire` 後の `approval retry` で absent として通る)
   - 切替後 `content_hash(plugins/<name>)` (symlink を辿る) を再計算し payload と一致することを確認 (不一致 → 旧状態へ戻し pending)
   - **プレーン live の検出は手順 1 の直後 (手順 0)**: `plugins/<name>` が dirfd+lstat でプレーン dir なら、版 (4) と git (5) までは通常どおり進め (ジャーナル無し)、`legacy_plain_present` を理由に pending + activity + 通知で終える。`afx plugin retire <name>` (flock 下、未完ジャーナルがあれば拒否、`plugins/<name>` → `plugins/_retired/<name>-<UTC ts>/` の 1 rename、activity `plugin_retired`) の後の `approval retry <id>` が手順 1 から流し直し、旧状態 absent として 6 へ進む
...
8. `candidate_origin='staging'` なら候補を削除、lock 解放。`human` は削除しない (人間所有)
...
- **git の失敗・版作成の失敗は稼働中の版を決して壊さない** (どちらも切替に先行するため)。**切替は原子操作 1 回** (rename) なので「live が無い瞬間」は無い。起こりうる中間状態は次の 3 つで、いずれも pending に統一され、再試行で収束する:
...
であり、`analyze_for_agent` (`corr_matrix`/`rolling_corr_summary`/
`lead_lag`) が行う DB 読み (`ohlcv_history`/`ohlcv_cache` からの内向き・
自傷性の読み取り) はこの節の対象外だった (§10 起票リストにも
`plugin max_bars × cache 保持期間` のみで analyze の読み窓は存在しない)。
`[[outbound-request-budget-is-a-design-constraint]]` の論点 (公開リポジトリ
で他人に損害を与える) はここには当たらない — 自機の DB 走査であり、害は
改善ループ自身の遅延と I/O であって外部への損害ではない。
...
4. **D6 変異列** (§5) を全て殺す / **経路 P3 (`bless --from _human`) は live の形で二分** (absent / symlink = pending + 証跡 + `preparing` ジャーナルを 1 tx → 切替 → approved / **プレーン = pending + 証跡のみ、ジャーナルも切替も決定も作らず `legacy_plain_present` pending**、codex 13 周目 M1) / **git サブプロセスが scheduler スレッドから呼ばれない**回帰テスト / 承認は git 記録成功後にしか `approved` にならない / **git 失敗を注入しても稼働中の旧版が消えない** (記録が切替に先行) / **切替は原子 (1 rename)** (2 段化の killer) / **live がプレーン dir の承認は版 + git 記録の後 `legacy_plain_present` で pending に留まり、`plugin retire <name>` (flock、未完ジャーナルで拒否、1 rename) → `approval retry` で absent → symlink として完了する。稼働中に plain を自動で入れ替えない** / **`flock` により別プロセス bless と競合しない** / **履歴は bare** (承認後に live symlink を壊す git 操作の面が無い) / **版キーは `artifact_hash`** (test 違いの版が区別される) / **承認済み symlink plugin を再改善できる** (`resolve()` 非依存) / **`PluginMeta.path` は版実体に固定され、approve は次回起動まで稼働中の plugin に影響しない** / **approval の全決定が `apply_decision` (1 tx: approval CAS + backlog 遷移 + ジャーナル終端) を通り、全 terminal decision が plugin flock を取る** / **後発決定 key は `(name, content_hash)`** / **`GC_ROOTS` は §5.1 の唯一の定義に従う** (ここで集合を再展開しない — codex 14 周目 M1。`legacy_plain_present` pending の版も root に含まれる)。**その集合に無い版だけが reconcile で消える** / **switch ジャーナル: 切替直後 crash → reject/別 hash approve が先に来ても live が旧状態 (symlink / absent) へ戻り DB と一致、同名の未完は 1 件、`preparing` 行が FS 効果より先** / **版ストアは不変 (0500/0400)、in-place 編集は discover のディレクトリ名 hash 照合で拒否、live への bless は無い → `materialize` → `_human` → 候補経路** / **bless (`--from _human`) は approve と同じライフサイクル (ゲート → pending 行 + 証跡 → 昇格 → `apply_decision`) で、通常 submit と同じ kind 別ゲートを通る。ジャーナルの有無と切替の要否は §5.1 の P3 表 (live の形で二分) に従う** (ここで無条件に「ジャーナル付き」と書かない — codex 14 周目 M1) / **journal-first・sweep-last の起動順序と `GC_ROOTS` (単一定義)** / **`plugin rollback` / `bless-version` コマンドが存在しない (R12 pin)** / **経路 P1 (submit) はジャーナルを作らない。P2 (approve) と P3 (bless) は §5.1 の P3 表どおり — live が absent / symlink のときだけ `preparing` を作り、プレーンのときは作らない** (codex 14 周目 M1) / **payload は `candidate_origin`+`candidate_path`、staging は終端決定で削除・human は残る、候補欠損は `candidate_missing` pending**
...
15. RPC 台帳 `OPEN → FROZEN → PERSISTED|PERSIST_FAILED|DISCARDED`、in-flight counter、RPC timeout、Mission timeout、commit 同時発生の race matrix
...
24. Tx-1 の backlog CAS (`BEGIN IMMEDIATE` + 条件付き UPDATE + `rowcount`) が唯一の線形化点。**CAS 勝者だけが「選択した課題に対するゲート・承認申請・backlog outcome」を進める**が、**Tx-1 で入る discoveries と、敗者の重複見送りレポート・run 終端は残る** (codex 13 周目 M3)
...
28. 3 経路を別々の SQL sequence として固定: **P1 submit** (staging / `_human`、pending + 証跡のみ・ジャーナル無し) / **P2 approve** (`preparing` ジャーナル → 版 → git → 切替 → decide) / **P3 bless --from _human** (live の形で二分: absent・symlink は 1 tx で pending + 証跡 + ジャーナル、プレーンは pending + 証跡のみ → `legacy_plain_present`)。**ジャーナルを作らないプレーン経路 (P2/P3 共通) の版 temp は `<artifact_hash>.tmp-approval-<approval_id>`、phase 更新なし、その approval が pending の間だけ所有、lock 下で同名 temp の hash を検証して頭から冪等に再実行 (不一致なら削除してやり直し)** — この temp の作成・fsync・rename・git の各 crash point を fault matrix に含める (codex 13 周目 I1 / 14 周目 M2)
...
- **【R12-(a′)、codex 11 周目 I1/I2】既存プレーン plugin の自動移行 (`RENAME_EXCHANGE` による plain↔symlink 原子交換・`retired_path` の巻き戻し・plain 専用 phase)** — 設計テキストは §A.1 (9 周目版) と §A.4 (10 周目版)。プラン 10 は「plain live への承認は `legacy_plain_present` pending、人間が `plugin retire` → `approval retry`」。手作り plugin が多く手動 retire が負担になったら再訪
...
- **risk gate 提案のポートフォリオ水準評価** — `max_positions` / `max_total_risk_pct` / `max_leverage` / `daily_loss_limit_pct` / `drawdown_kill_pct` 等は単一 strategy 再生では評価できず、strategy 集合の manifest hash・同時 intent の順序・複数 timeframe を持つポートフォリオ harness が要る。プラン 10 では risk_gate 提案全体が `unsupported_in_plan10` (R12-(b)。`unsupported_param` の粒度は §A.2 の設計) (codex 6 周目 I7)
...
| 13 | 2026-08-17 | codex (gpt-5.6-sol) | **C0 / I1 / M3 = 4 件** (11 周目 8 件: 閉じた 7 / 部分 1、12 周目 5 件: 閉じた 4 / 部分 1) | **全件採用** (`codex-round13.md`)。**I1 ジャーナルを作らない legacy plain 経路の版 temp identity を `approval-<approval_id>` に定め、phase 更新なし・pending の間だけ有効・冪等再実行と定義** (§5.1-4 に経路別の表を追加) / M1 §7.1-4 と Task 11 の P3 を live 形で二分 / **M2 版・履歴の不変条件を一方向 (`approved ⇒ 版化 + 記録`) へ統一** (逆は保証しない — ゲート済み pending が版・履歴に在るのは正常) / M3 §8.1-24 を「勝者は選択課題のゲート・承認・outcome を進める。discoveries と敗者レポートは残る」へ。適用は指揮者 (opus) |
| **14** | 2026-08-17 | codex (gpt-5.6-sol) | **C0 / I0 / M2** (11 周目: 閉じた 7 / 部分 1、13 周目: 閉じた 2 / 部分 2) — **収束判定「設計として実装計画へ渡せる状態である」** | **全件採用** (`codex-round14.md`)。M1 §7.1-4 後半の短縮 3 箇所 (bless のジャーナル無条件化・「P2/P3 だけが `preparing`」・`GC_ROOTS` の再展開) を §5.1 の正規定義への参照に置換 / M2 §8.1-28 に journal-less プレーン経路の版 temp identity (`approval-<approval_id>`)・phase 更新なし・pending 中のみ所有・冪等再実行を追記。**以後の指摘は SQL / fault matrix / fixture / 実機 E2E の粒度 → 実装計画へ** |
...
### A.4 (a′) 11 周目で撤去した「既存プレーン plugin の自動移行」(10 周目 R12 版: RENAME_EXCHANGE + `retired_path`)

> - **既存のプレーンな `plugins/<name>/` (手作り、本プラン以前)** は自動移行しない。discover は両形 (プレーン / symlink) を読み、両者は共存できる。**同名の候補が初めて承認されたとき**、プレーン dir は `RENAME_EXCHANGE` で symlink と入れ替わり、退いたプレーン dir は **`plugins/_retired/<name>-<UTC タイムスタンプ>/` へ rename** される (人間所有。自動では削除しない)。**旧版の版化・履歴記録は行わない**
> - ジャーナル: `old_kind IN ('absent','symlink','plain')`、`retired_path = plugins/_retired/<name>-<ts>` (old_kind=plain のときだけ) を最初の行に確定
> - 切替 (プレーン): `renameat2(AT_FDCWD, temp, AT_FDCWD, plugins/<name>, RENAME_EXCHANGE)` (ctypes syscall、Linux 3.15+、同一 FS) で temp symlink と live ディレクトリを 1 回で交換 → temp パスに来た旧ディレクトリを `retired_path` へ `rename`。**`RENAME_EXCHANGE` 非対応 (kernel/FS) は初回切替を fail closed** (理由 `exchange_unsupported`)
...
| 2026-09-12 | §4.1 追記 (T3/T5) | `missions.finished_at` が論理終端時刻 (worker 結果到着時刻、親ゲート分を含まない) である旨を明記し、壁時計が要る場面は `activity` を使うよう追記。`plugins/_archive/INDEX.md` の既存ファイルへのヘッダ後付けは据え置き (現状維持) と明記 — いずれもコード変更なし、裁定の記録のみ | test-hygiene 設計書 `2026-09-12-test-hygiene-design.md` §T3/§T5、指揮者裁定 (2026-09-12) | - |
| 2026-09-12 | §1.1・§1.4 訂正 (C6) | codex+chatgpt の `chatgpt_subscription_active_until` 起動時検査 (§1.1 の provider 説明・§1.4 の起動時検査④) を「撤去済み」に書き換え。実 `~/.codex/auth.json` にこのキーが無く空振り WARNING しか出なかったため (test-hygiene T4 で撤去)。期限切れ・利用上限は codex 自身の実行時 stderr (`usage limit`/`try again at`/`401 Unauthorized`) を `cli_stderr_fatal` で検知する側に寄せた旨を明記 | codex 1 周目 Important 是正依頼 (`tmp/review-20260912-th/codex-r1.md`)、test-hygiene T4 撤去 (`a6cef16`) | - |
