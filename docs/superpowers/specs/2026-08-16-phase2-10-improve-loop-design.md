...
- **wave と slot の durable 状態機械 (codex 4 周目 I4 / 5 周目 I1)**: 新テーブル `improve_waves(period_key TEXT PRIMARY KEY, created_at TEXT NOT NULL, expected INTEGER NOT NULL)` と **`improve_wave_slots(wave_period_key TEXT NOT NULL REFERENCES improve_waves(period_key), k INTEGER NOT NULL, status TEXT NOT NULL CHECK(status IN ('reserved','running','done','failed')), mission_id INTEGER, created_at TEXT NOT NULL, updated_at TEXT NOT NULL, PRIMARY KEY(wave_period_key, k))`**。手順: ①`M = min(improve.parallel, 空きスロット数)`。**M=0 なら何も書かない** (period 非消費、次 tick で再試行) ②**1 つの短い tx** で `INSERT OR IGNORE INTO improve_waves(period_key, created_at, expected) VALUES (?, ?, M)` (`rowcount=1` = 起動権) + **同じ tx で slot 行 `k=0..M-1` を `reserved` で INSERT** — **「wave 行が存在する = period 消費」** (行と slot は原子的に生まれる) ③`open|observation` の backlog id を不変スナップショットとして取り partition `k=0..M-1` を分ける ④supervisor は Mission を起動するとき **slot を CAS で claim** (`UPDATE improve_wave_slots SET status='running', mission_id=?, updated_at=? WHERE wave_period_key=? AND k=? AND status='reserved'`) — **これを §4.1 Tx-0 (missions INSERT + run INSERT) と同じ tx で行う**。Mission が終端したら slot を `done` / `failed` に (Tx-2 / 補償 tx / 起動時回収と同じ tx) ⑤`submitted` は列で持たず **`status != 'reserved'` の slot 数から導出** ⑥**部分 submit** (spawn 失敗等): 未 claim の slot は `failed` (activity に記録)、period は消費済み (行がある) ⑦**起動時**: `reserved` の slot (claim 前に crash) は **同じ period_key の wave を再開**して claim → 起動 (新しい CAS は不要、二重起動もしない — 旧 Mission が無いので) / `running` の slot で mission が interrupted → `failed`。**「spawn の瞬間」を exactly-once と見なさない** — 線形化点は slot の claim CAS。**手動 `improve` は wave/slot 行を作らない**
...
6. **レポート (親だけが書く)**: `artifact.type=report`、およびゲート不合格・評価不能・observation・敗者のとき、`reports/` に書く。**`proposal_kind=risk_gate` の report は harness 所有の評価が成立したときだけ report として成立する** (codex 4 周目 I7 / 5 周目 I3 — 本体設計書 §6「risk gate パラメータの提案は approval を出さないが、バックテスト必須・最低取引数・**out-of-sample 比較**の規律はレポートの根拠にも同様に適用」): 親が `risk_gate_proposal.pairs` の各 pair について、承認済み strategy 集合を対象に **baseline (現行 risk 設定) と候補 (`risk_gate_proposal.params` を上書きした設定スナップショット) の両方**で ①`run_in_sample(..., record_fn=<sink>)` ②**ハーネス所有の固定 holdout `run_holdout_gate(..., record_fn=<sink>)`** を回す (既存 `settings` 引数に override を渡すだけで新経路は作らない)。**in-sample の合計取引数 ≥ 30 なら in-sample と holdout (OOS) の baseline/候補比較をレポートに添付**して成立、**< 30 なら「評価不能」→ report を書かず observation** (`insufficient_trades`)。両実行の行は Tx-2 で `backtest_runs` に保存 (`scope=in_sample` / `holdout_gate`, `issued_by=harness`)。**holdout の内容は worker 出力・RPC 返却・次回注入コンテキストのどこにも出さない** (遮断 8 — 親専有の report と監査行だけ)。`core` / `research` の report は評価を要さない。
   **書き方 (codex 5 周目 I6 — 失敗 Mission の部分/孤児 report を残さない)**: 本文は tx 外で **親専有の一時領域 `reports/.tmp/improve-<mission_id>.md.part`** に `O_WRONLY|O_CREAT|O_EXCL|O_NOFOLLOW` (dirfd) で書き `fsync`。**最終名 `reports/improve-YYYY-MM-DD-<mission_id>.md` への `rename` (公開) は Tx-2 の COMMIT の後、最後の一手** — Tx-2 には**予定の最終パス**を `report_path` として書く。公開 rename に失敗 (最終名が既に在る = 事前に置かれた通常ファイル/symlink、`O_EXCL` 相当は `renameat2(RENAME_NOREPLACE)` で担保) → **補償 tx** で `improvement_runs.result=NULL`、backlog `observation` `report_failed:<reason>` (§4.3)、activity + 通知。**Tx-2 が rollback したら `.part` を削除**。**起動時 reconcile**: `reports/.tmp/*.part` を全削除し、命名規則に一致する最終 report のうち**どの FINISHED run の `report_path` からも参照されないもの**を削除する (収束規則: report が在る ⇔ それを指す finished run が在る)。`reports/` と `reports/.tmp/` は起動時に親が mkdir、gitignore 済み、**worker の rw には無い** (§2.1)。**agent の `body_md` は「提案本文」節に引用として入れる — 信用しない**
7. **`improvement_runs.finish` (Tx-2)**: 承認申請を出したら `result='approval'` + `approval_id` (+ レポートの一時ファイルが書けていれば予定 `report_path`)。承認申請なしで**レポートの一時ファイル作成に成功したときだけ** `result='report'` + 予定 `report_path`。一時ファイル作成に失敗していたら `result=NULL`。**公開 (rename) が COMMIT 後に失敗したら補償 tx で `result=NULL` に戻す** (codex 2 周目 M2 / 5 周目 I6 — DB が存在しない report を指さない)
...
| selected | **レポート作成に失敗** (承認申請なし) | observation | `report_failed:<safe_reason>` |
| selected | ゲート不合格 / 評価不能 (<30、strategy 採用ゲートまたは risk_gate report の評価) / artifact=observation | observation | `gate_failed:<reason>` / `insufficient_trades:<n>` / `observation:<reason>` |
...
- 稼働中の `plugins/<name>` は**版ディレクトリを指す相対 symlink** (`.versions/<name>/<artifact_hash>`)。ロールバック = symlink を別の版へ向け直す (`plugin rollback <name> <artifact_hash>`。**git checkout は使わない**。**対象は「その `(name, content_hash)` の最新決定が approved (D4 admission) で、かつ版ディレクトリが在る」版だけ** — adopted のみで承認の無い版・後発 reject 済みの版へは fail closed (メッセージ「bless で明示承認を作ること」)。**rollback は approval の terminal decision ではなく、監査付きの別操作**: 同じ plugin flock 下で symlink を 1 rename で切り替え、`plugin_versions` に `origin='rollback'` 行 (`INSERT OR IGNORE`) と activity `plugin_rollback` を残す。codex 5 周目 I5)。**discovery は symlink を辿った先の版ディレクトリを `PluginMeta.path` に固定し、稼働中サービスは再起動まで同じ版を使い続ける** (§2.3、codex 3 周目 I4) — 切替は次回起動/再読込にだけ効く
...
1. **排他 + 昇格ジャーナルの確認**: **`flock` を `plugins/.locks/<name>.lock` に取得** (プロセス間 — サービスの approve / **reject / expire / invalidate** / 起動時 reconcile / `plugin rollback` / 別プロセスの `afx plugin bless` が**全て**同じ lock を取る、codex 4 周目 I5) + プロセス内は plugin 名ごとの `threading.Lock` (ⓒ、順序は flock → thread lock)。lock 取得後に approval が `pending` であること・後発決定・**この `(name, content_hash)` に未完のジャーナルが無いこと**を再確認してから FS 副作用に進む (未完があれば先に reconcile — 下記)。異なる plugin の並行承認は 5.2 の CAS が守る
   **昇格ジャーナル `plugin_promotions` (codex 5 周目 I2 — `flock` は crash を越えない)**: 新テーブル `plugin_promotions(approval_id INTEGER PRIMARY KEY, name TEXT NOT NULL, content_hash TEXT NOT NULL, old_target TEXT, new_target TEXT NOT NULL, phase TEXT NOT NULL CHECK(phase IN ('versioned','recorded','switched','decided')), updated_at TEXT NOT NULL)`。**phase の書込は次の FS 効果より先、lock 下の短い tx で行う**: 版ディレクトリ作成が完了したら行を INSERT (`phase='versioned'`、`old_target` = 現 live のリンク先文字列 or NULL、`new_target` = 新版) → 履歴記録が完了したら `recorded` → **切替の直前に `switched` を書いてから** rename/exchange → `apply_decision` と同じ tx で `decided`。**収束規則: ジャーナルが `decided` ⇔ live symlink と DB 決定が一致**。未完ジャーナルの扱い: (a) **approve 再試行 / 起動時 reconcile** は phase から再開して完了させる (hash がまだ一致し後発 reject が無ければ) (b) **reject / expire / invalidate が来た**とき未完ジャーナルがあれば、reconcile が**切替を `old_target` へ巻き戻し** (symlink を 1 rename で戻す。`old_target` が NULL = 元々無かったなら symlink を除去) → ジャーナルを閉じる → その後に reject を適用する。これで「DB は rejected・live は新版」が起きない
...
4. **版ディレクトリの作成 (冪等)** (完了後にジャーナル `versioned`): `plugins/.versions/<name>/<artifact_hash>.tmp-<approval_id>/` に 3 ファイルをコピー → 各ファイルとディレクトリを `fsync` → `rename` で `plugins/.versions/<name>/<artifact_hash>/` へ (既に同 artifact_hash の版があれば作らない)。失敗 → pending + 通知。**本番 symlink には触れていない**
5. **旧プレーン版の保全 (初回移行のときだけ、codex 3 周目 I1 / 4 周目 I6)**: `plugins/<name>` が**プレーンなディレクトリ**なら、lock 下でその 3 本をスナップショット検査 → `artifact_hash_old` / `content_hash_old` を計算 → **`plugins/.versions/<name>/<artifact_hash_old>/` へコピー** (4 と同じ tmp → fsync → rename、冪等) → **bare 履歴へ「adopted pre-existing」として記録** (5.2 の plumbing、commit message に `adopted pre-existing content=<…> artifact=<artifact_hash_old>`。既に同 tree なら no-op) → **`plugin_versions` に `(name, artifact_hash_old, content_hash_old, origin='adopted', approval_id=NULL)` の行を短い tx で書く** (GC root — 下記)。これで旧版も版ストア・履歴・DB に残り、`plugin rollback` で戻れる
6. **履歴への記録 (ⓑ) — 新版ディレクトリの 3 本から、`<name>/<file>` のパスで** (完了後にジャーナル `recorded`): 下記 5.2 の blob-level plumbing。失敗 (git 不在 / detached / identity 無し / CAS 上限) → **pending のまま + 通知。本番には一切触れていない**
7. **切替 (原子操作 1 回)** (直前にジャーナル `switched` を短い tx で書く — FS 効果より先): temp symlink `plugins/.<name>.link-<approval_id>` (→ `.versions/<name>/<artifact_hash>`) を作り、`plugins/<name>` の現在の形で分岐:
...
8. **`apply_decision(status="approved")`** (§4.3 — approval 行 CAS + backlog `done` + **`plugin_versions` 行 `(name, artifact_hash, content_hash, origin='approval', approval_id)`** + **ジャーナル `decided`** を 1 tx で。決定は最後)。ここで初めて次回起動の `approved_plugins()` に載る
9. staging を削除、lock 解放

**却下・期限切れ・失効**: 同じ plugin `flock` 下で、未完ジャーナルがあれば先に巻き戻し (1 の (b)) → `apply_decision` (backlog を `observation`) + staging 削除。ジャーナルが無ければ `plugins/<name>`・版・履歴には触れない。

**版の台帳 `plugin_versions` (codex 4 周目 I6)**: 新テーブル `plugin_versions(name TEXT NOT NULL, artifact_hash TEXT NOT NULL, content_hash TEXT NOT NULL, origin TEXT NOT NULL CHECK(origin IN ('approval','adopted','rollback')), approval_id INTEGER, created_at TEXT NOT NULL, PRIMARY KEY(name, artifact_hash))`。決定/adopt/rollback と**同じ tx** で書く。**GC の root 集合 = `plugin_versions` の行 ∪ live symlink の指す先**。**この表は保管/GC の provenance であって admission の根拠ではない** (codex 5 周目 I5) — `plugin rollback` の許可判定は D4 (最新決定が approved の `(name, content_hash)`) と版の存在で行い、この表は rollback 実行時に `origin='rollback'` の行 (`INSERT OR IGNORE`) を追記する記録先にすぎない。
...
- **起動時 reconcile の位置**: `init_db` と中断 Mission の回収より後、**`approved_plugins()` より前** (後だとその起動では復旧した承認がロードされない — codex 4 周目 I3)。同時に ①§2.3 の孤児 staging 掃除 ②`.versions/<name>/*.tmp-*` の削除 ③**`plugin_versions` に行が無く、かつどの live symlink も指していない**版ディレクトリの削除 (orphan。approval payload だけを root にしない — プラン 10 以前の approval は `artifact_hash` を持たず、adopt した旧版が消える — codex 4 周目 I6) ④**temp link `.<name>.link-*` の残骸削除** ⑤**exchange 後に temp パスへ来た旧プレーン dir の残骸** — その dir の `artifact_hash` を計算し、`.versions/<name>/<その hash>/` が無ければ **同じ adopt (5.1-5) を再開** (版作成 + 履歴記録) してから削除、既に版があれば一致確認して削除、不一致なら ERROR で残す ⑥**未完の `plugin_promotions` (phase != decided) を phase から完了または `old_target` へ巻き戻して閉じる** (5.1-1) ⑦`plugins/<name>` が dangling symlink なら activity ERROR (人間の操作を待つ。自動では向け直さない) を行う。git 不在・reconcile 失敗はサービス起動を止めない (警告 + pending のまま)
...
| C | 8 | バックログ拡張 (`observation` / attempts / last_result / **選択 CAS / 状態機械ヘルパ `apply_approval_outcome` / `approvals.apply_decision`** / `backlog reject\|reopen`) + `improvement_runs.mission_id` (部分 UNIQUE) + run lifecycle (Tx-0 = missions+run+slot の 1 tx / Tx-1 bind / 全終端で finish) + `improve_waves` + `improve_wave_slots` + `plugin_versions` + `plugin_promotions` テーブル + 起動時回収 (interrupted → observation、`finished_at IS NULL` のみ、reserved slot の再開・running+interrupted は failed) + store helper の `commit=False` 変種 (`missions.start` / `decide` / `expire_due` / `missions.finish` 含む) + 注入コンテキスト生成 + prompt | §3.1/§3.2/§4.1/§4.3/§5.1 | — |
| **D** (loop) | 9 | `ImproveSupervisor` (N スロット・wave 状態機械・slot 専用 write 接続 / dispatcher 専用 RO 接続・shutdown/join + CLI pgid 回収) + scheduler の「最新 occurrence の period key」/ wave+slot 同一 tx 作成 / slot claim CAS / catch-up + `improve` / `improve add` / `backlog` / `policy add` コマンド (**`improve` の有効化配線は Task 12**) | §3.1 | 8 |
| D | 10 | `ImproveLoop` (三相 + `ImproveRunContext` + source snapshot (固定 `PluginMeta.path` から、`_examples` 込み) + Tx-0/Tx-1/Tx-2 (missions.finish 込み) + 補償 tx + commit 相ゲート + 担当集合検査 + risk_gate report 評価 (in-sample + holdout、baseline/候補) + 台帳永続化 + 承認申請 + レポート (`.tmp/*.part` → COMMIT 後 rename 公開、失敗時 result=NULL + report_failed、起動時 orphan 掃除) + backlog 遷移) | §4 | 6, 7, 8 |
| **E** (承認) | 11 | 版ディレクトリ (`artifact_hash`) + bare 履歴 (blob-level plumbing) + **旧プレーン版の adopt (+ `plugin_versions` 行)** → symlink 切替 (rename / `RENAME_EXCHANGE`) + 全 terminal decision の `flock` + **昇格ジャーナル `plugin_promotions` (phase 前書き・巻き戻し)** + reconcile (孤児 staging・tmp 版・`plugin_versions`/symlink 非参照版・temp link・旧 dir 残骸の adopt 再開・未完ジャーナル・dangling) + bless 経路 (初回 symlink 化) + **`approval retry` / `plugin rollback` (D4 approved 版のみ・非 decision・監査行) の handler と配線** + 全決定経路の `apply_decision` 化 (key `(name, content_hash)`) | §5 | 6, 8 |
...
52. 昇格ジャーナル `plugin_promotions` の DDL/phase と、切替直後 crash → reject/expire/reconcile の multi-process matrix。CAS 敗者は live を変えない
