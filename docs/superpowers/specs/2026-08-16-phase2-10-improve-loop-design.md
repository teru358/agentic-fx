...
| R11 | **サブスク backend (claude / codex+chatgpt) は「agent 自身が scratch の OAuth 認証コピーを読み、shell+ネットワークで持ち出せる」というハーネス固有の残余リスクを受容する** (codex 4 周目 C1)。従量課金鍵が無い不変条件は保つが、サブスク認証トークンは agent から可読。fail closed にしない。codex+llama_swap / local にはこの資格情報が無い。恒久対処 (別 UID / credential broker / 短命トークン) は起票 | ユーザー裁定 2026-08-16 |
...
- **起動時検査** (`service.build_app` の既存 `_check_llama_swap` の隣): 選択された backend について ①`bin` を `shutil.which` → `realpath` で**絶対パスへ正規化**し、実行可能な通常ファイルであること。**codex は ELF (vendor native) を要求し、node ラッパ (`codex.js` / shebang スクリプト) は拒否** — エラー文言に探し方 (`npm root -g` 配下 `@openai/codex/node_modules/@openai/codex-linux-x64/vendor/.../bin/codex`、または `codex --version` を打つラッパの中身) を書く (codex 3 周目 I8。node ラッパの closure は起票 §10) ②`<bin> --version` が 0 で返る (rlimit なし・親プロセス側で) ③認証ファイルが存在する ④codex+chatgpt は `chatgpt_subscription_active_until` を読んで 7 日以内なら WARNING、過ぎていれば ERROR ⑤**improve+claude のとき、サービス自身の初期 env (`/proc/self/environ`) に秘密名パターン (`*_API_KEY` / `*TOKEN*` / `*SECRET*` / `*WEBHOOK*` / `ANTHROPIC_*` / `OPENAI_*`) の変数があれば起動拒否** — 理由: claude worker は `/proc` を読めるため親の初期 env を読み戻せる (R10)。文言は「秘密は exported env でなく `.env` に置くこと」(`python-dotenv` は `os.environ` に setenv するだけで `/proc/self/environ` には現れない)。**①②③⑤のいずれかを欠けば `build_app` は起動拒否 (fail closed)** — Landlock 不可時の improve 起動拒否と同じ扱い。backend=local の環境では一切走らない
...
- **wave の状態機械 (I4)**: 新テーブル `improve_waves(period_key TEXT PRIMARY KEY, created_at TEXT NOT NULL, status TEXT NOT NULL CHECK(status IN ('reserved','running','completed')), expected INTEGER NOT NULL, submitted INTEGER NOT NULL DEFAULT 0)`。手順: ①`M = min(improve.parallel, 空きスロット数)`。**M=0 なら何も書かない** (period 非消費、次 tick で再試行) ②**1 つの短い tx** で `INSERT OR IGNORE INTO improve_waves(period_key, created_at, status, expected, submitted) VALUES (?, ?, 'reserved', M, 0)` (`rowcount=1` = 起動権) + slot 予約 (in-memory、同じ critical section) ③`open|observation` の backlog id を不変スナップショットとして取り partition `k=0..M-1` を分ける ④各 Mission の submit が成功するたび `UPDATE improve_waves SET submitted=submitted+1, status='running' WHERE period_key=?`。全部 submit できたら `completed` ⑤**部分 submit** (spawn 失敗等): 未 submit の partition は activity に記録し、この period は**消費** (`submitted ≥ 1` なら completed 扱い) ⑥**起動時**: `status='reserved' AND submitted=0` の行は**削除** (0 件実行で period を失わない — 次 tick で再 CAS)、`submitted ≥ 1` の行は `completed` に確定 (partial は消費の規則)。`running` の Mission 自体は `missions.recover_interrupted` が回収する
...
```json
{
  ...
  "artifact":     {"type": "plugin", "name": "^[a-z][a-z0-9_]{0,63}$", "kind": "indicator"|"signal"|"strategy",
               ...
               |  {"type": "report", "proposal_kind": "core"|"risk_gate"|"research", "title": str, "body_md": str,
                   "risk_gate_proposal": {"params": {<risk.* のサブセット>}, "pairs": [str]} | null}   // risk_gate のとき必須 (§4.2-6)
...
- **Tx-0 (prepare、短い)**: missions 行 + `improvement_runs.start(mission_id, backlog_id=NULL, commit=False)` → COMMIT。**run の生成時点は Mission と同じ** — 出力不正・timeout・worker 異常死でも履歴 (run) が残る
...
6. **レポート (親だけが書く、tx 外)**: `artifact.type=report`、およびゲート不合格・評価不能・observation・敗者のとき、`reports/` に書く。**`proposal_kind=risk_gate` の report は harness 所有の評価が成立したときだけ report として成立する** (codex 4 周目 I7 — 本体設計書 §6「risk gate パラメータの提案は approval を出さないが、バックテスト必須・最低取引数・out-of-sample の規律はレポートの根拠にも同様に適用」): 親が `risk_gate_proposal.pairs` の各 pair について、**現行の paper backtest 設定スナップショットに `risk_gate_proposal.params` を上書き**した settings で `run_in_sample(..., record_fn=<sink>)` を回し (承認済み strategy 集合を対象。`run_in_sample` の既存 `settings` 引数に override を渡すだけで新経路は作らない)、**合計取引数 ≥ 30 なら in-sample 指標と現行設定での同指標 (baseline) をレポートに添付**して成立、**< 30 なら「評価不能」→ report を書かず observation** (`insufficient_trades`)。**holdout は回さない** (採用でなくレポートなので)。`core` / `research` の report は評価を要さない。`reports/improve-YYYY-MM-DD-<mission_id>.md` を親が整形して書く。**書き方**: `reports/` の dirfd に対し `openat(name, O_WRONLY|O_CREAT|O_EXCL|O_NOFOLLOW)` — 既存パス (通常ファイルであれ symlink であれ) が在れば **fail closed** (activity + 通知、Mission 処理は続行、backlog は `observation` `report_failed:<reason>` — §4.3)。`reports/` は起動時に親が mkdir、gitignore 済み、**worker の rw には無い** (§2.1)。**agent の `body_md` は「提案本文」節に引用として入れる — 信用しない**
7. **`improvement_runs.finish` (Tx-2)**: 承認申請を出したら `result='approval'` + `approval_id` (+ レポートも書けていれば `report_path`)。承認申請なしで**レポートファイルの作成に成功したときだけ** `result='report'` + `report_path`。**レポート作成に失敗したら `result=NULL` + `finished_at` + activity に理由** (codex 2 周目 M2)
8. **backlog 遷移 (Tx-2、§4.3)** と `last_result`
9. **`missions.finish(commit=False)` を Tx-2 に含めて `COMMIT` → 掃除**: 承認申請を出した staging は残す。それ以外は削除。台帳 `PERSISTED`。接続 close
...
- 稼働中の `plugins/<name>` は**版ディレクトリを指す相対 symlink** (`.versions/<name>/<artifact_hash>`)。ロールバック = symlink を別の版へ向け直す (`plugin rollback <name> <artifact_hash>`。**git checkout は使わない**)。**discovery は symlink を辿った先の版ディレクトリを `PluginMeta.path` に固定し、稼働中サービスは再起動まで同じ版を使い続ける** (§2.3、codex 3 周目 I4) — 切替は次回起動/再読込にだけ効く
...
1. **排他**: **`flock` を `plugins/.locks/<name>.lock` に取得** (プロセス間 — サービスの approve / **reject / expire / invalidate** / 起動時 reconcile / `plugin rollback` / 別プロセスの `afx plugin bless` が**全て**同じ lock を取る、codex 4 周目 I5) + プロセス内は plugin 名ごとの `threading.Lock` (ⓒ、順序は flock → thread lock)。lock 取得後に approval が `pending` であることと後発決定を再確認してから FS 副作用に進む。異なる plugin の並行承認は 5.2 の CAS が守る
...
6. **履歴への記録 (ⓑ) — 新版ディレクトリの 3 本から、`<name>/<file>` のパスで**: 下記 5.2 の blob-level plumbing。失敗 (git 不在 / detached / identity 無し / CAS 上限) → **pending のまま + 通知。本番には一切触れていない**
7. **切替 (原子操作 1 回)**: temp symlink `plugins/.<name>.link-<approval_id>` (→ `.versions/<name>/<artifact_hash>`) を作り、`plugins/<name>` の現在の形で分岐:
...
8. **`apply_decision(status="approved")`** (§4.3 — approval 行 CAS + backlog `done` + **`plugin_versions` 行 `(name, artifact_hash, content_hash, origin='approval', approval_id)`** を 1 tx で。決定は最後)。ここで初めて次回起動の `approved_plugins()` に載る
9. staging を削除、lock 解放

**却下・期限切れ・失効**: 同じ plugin `flock` 下で `apply_decision` (backlog を `observation`) + staging 削除。`plugins/<name>`・版・履歴には触れない。

**版の台帳 `plugin_versions` (codex 4 周目 I6)**: 新テーブル `plugin_versions(name TEXT NOT NULL, artifact_hash TEXT NOT NULL, content_hash TEXT NOT NULL, origin TEXT NOT NULL CHECK(origin IN ('approval','adopted','rollback')), approval_id INTEGER, created_at TEXT NOT NULL, PRIMARY KEY(name, artifact_hash))`。決定/adopt/rollback と**同じ tx** で書く。**GC の root 集合 = `plugin_versions` の行 ∪ live symlink の指す先**。`plugin rollback <name> <artifact_hash>` は**この表に行がある版にしか向けられず**、実行時に `origin='rollback'` の行 (既に行があれば `INSERT OR IGNORE`) と activity を残す。
...
| **B** (柵) | 5 | `landlock.execute_paths` + backend 別 exec closure + `_bootstrap_improve_profile` 拡張 (`/dev` rw, `/proc` (claude), resolve, **handshake `mission_id`/`staging_dir`/`source_snapshot_dir` の受領・相互照合・rw 化**) + assert 拡張 + env 追加 + `/dev/null` O_RDONLY + **`discover` の `_`/`.` 除外と名前正規形 + symlink 追従 + `PluginMeta.path` を版実体に固定** | §2 | — |
...
| C | 8 | バックログ拡張 (`observation` / attempts / last_result / **選択 CAS / 状態機械ヘルパ `apply_approval_outcome` / `approvals.apply_decision`** / `backlog reject\|reopen`) + `improvement_runs.mission_id` + run lifecycle (Tx-0 start / Tx-1 bind / 全終端で finish) + `improve_waves` (status/expected/submitted) + `plugin_versions` テーブル + 起動時回収 (interrupted → observation、`finished_at IS NULL` のみ、reserved wave の削除) + store helper の `commit=False` 変種 (`decide` / `expire_due` / `missions.finish` 含む) + 注入コンテキスト生成 + prompt | §3.1/§3.2/§4.1/§4.3/§5.1 | — |
| **D** (loop) | 9 | `ImproveSupervisor` (N スロット・wave 状態機械・slot 専用 write 接続 / dispatcher 専用 RO 接続・shutdown/join + CLI pgid 回収) + scheduler の「最新 occurrence の period key」/ `improve_waves` CAS / catch-up + `improve` / `improve add` / `backlog` / `policy add` コマンド (**`improve` の有効化配線は Task 12**) | §3.1 | 8 |
| D | 10 | `ImproveLoop` (三相 + `ImproveRunContext` + source snapshot (固定 `PluginMeta.path` から、`_examples` 込み) + Tx-0/Tx-1/Tx-2 (missions.finish 込み) + 補償 tx + commit 相ゲート + 担当集合検査 + risk_gate report 評価 + 台帳永続化 + 承認申請 + レポート (O_EXCL\|O_NOFOLLOW、失敗時 result=NULL + report_failed) + backlog 遷移) | §4 | 6, 7, 8 |
| **E** (承認) | 11 | 版ディレクトリ (`artifact_hash`) + bare 履歴 (blob-level plumbing) + **旧プレーン版の adopt (+ `plugin_versions` 行)** → symlink 切替 (rename / `RENAME_EXCHANGE`) + 全 terminal decision の `flock` + reconcile (孤児 staging・tmp 版・`plugin_versions`/symlink 非参照版・temp link・旧 dir 残骸の adopt 再開・dangling) + bless 経路 (初回 symlink 化) + **`approval retry` / `plugin rollback` の handler と配線** + 全決定経路の `apply_decision` 化 (key `(name, content_hash)`) | §5 | 6, 8 |
...
- **A / B / C-8 は worktree 並列**可 (codex 4 周目 M4: **D-9 は C-8 の後** — 並列集合から外す)。C-7 は B-5 の後。D-10 は B-6・C-7・C-8 の後。E-11 は B-6・C-8 の後 (A と並列可)。**F-12 は 1〜11 の後、F-13 は 12 の後**
...
43. risk-gate report の proposal schema、in-sample/baseline 添付、評価不能時 observation を本体設計書 §6 と逐語対応 (holdout は回さない)
