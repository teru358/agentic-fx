...
| **`params` 自体が `dict` でない場合、および未知キーの型が混ざっている場合 (v1.3f、3 周目 codex Important)** | **`params` が `dict` でなければ、キー集合の検査より前に `ValueError` を投げる**。文言は `params. must be a mapping, got <型名>` (キー名の代わりに `.` の直後に何も置かず、他の例外と同じ `params.` 接頭辞を保つ)。未知キーの選択は**型を跨いで比較しない決定的な順序**で行う (`str` キーを先に通常の辞書順、`str` でないキーを後に `repr` 順) — `str` でないキーは loader が str キーしか通さない契約なのでそれ自体を未知キー扱いにする | `set(params)` は `params` が非 iterable だと `TypeError`、`sorted(set(params) - _KNOWN_PARAMS)` は未知キーの型が混ざる (例 `{0: 1, "typo": 1}`) と `int` と `str` の比較で `TypeError` になる。どちらも本節の「不正な params は `ValueError`」規約を破って生の `TypeError` を漏らす (3 周目 codex terra 実測) |
...
| 日付 | 版 | 変更 | 理由 | commit |
|---|---|---|---|---|
| 2026-09-19 | v1.3f | **3 周目レビュー (絞り込み、codex terra 1 束) の是正を反映。Critical 0 / Important 1。** **§4**: `_reject_unknown_params` が `set(params)` / `sorted(...)` を無条件に呼んでおり、`params` が非 dict (`None` 等) や未知キーの型が混ざる場合 (`{0: 1, "typo": 1}`) に規約どおりの `ValueError` ではなく生の `TypeError` を漏らしていた。**規則表に「`params` 自体が非 mapping / 未知キー型混在」の行を追加** — 非 mapping は `params.` 接頭辞のまま `ValueError`、未知キー選択は型を跨いで比較しない決定的な順序 (str 優先 + repr) にする。9 本の `plugin.py` を同形で是正、自己テストに負例を追加 (160 → 214 passed) | 3 周目 codex terra Important (`tmp/review-20260919-iis/r3/codex-1.md`) | 本行の docs commit |
