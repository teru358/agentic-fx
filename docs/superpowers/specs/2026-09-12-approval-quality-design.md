...
| 2026-09-12 | v1.3 | §A 質検査は `_finalize_success` の BEGIN IMMEDIATE 内で行い、hit 時は rollback → observation (parallel ≥ 2 の二重申請を防ぐ)、降格候補の gate 行も `observation` で残し reason に pair / §D stdin 渡しは CliRunner 基底の既定 (argv に prompt が残れば fail closed、opencode は明示 opt-out) / 壊れた metrics_json は warning で fail-open / FLOAT_TOL 比較を `db.values_match` に統合 | `/code-review high` 2 周目 8 件 (`tmp/review-20260912-aq/code-review-r2.md`、must-fix 1) | 9acc651 |
