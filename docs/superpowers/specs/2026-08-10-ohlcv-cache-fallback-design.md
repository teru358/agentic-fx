...
## 改訂 5 (2026-08-11) — `ohlcv` 分割に伴う差分。**この節が以降の本文に優先する**

**本体設計書 §12 の改訂により、`ohlcv` は `ohlcv_cache` (ライブ逐次キャッシュ) と `ohlcv_history` (バックテスト・履歴分析) の 2 テーブルに分割された** (ユーザー承認 2026-08-11)。動機は「キャッシュに保持ポリシー (DELETE) を入れると、`mt5-live` と `mt5` が 1 文字違いのため source を見ない prune が履歴を無音破壊する」ことを構造で防ぐこと。

**本書の設計そのもの (窓計算 / 要求 interval への floor / source 単位の例外隔離 / `lookback_days` 配線) は分割後もそのまま成立する。** 変わるのは**永続化の境界**だけである。以下を読み替える:

| 本文の記述 | 改訂 5 での読み替え |
|---|---|
| `_cached_bars` が `ohlcv.load_bars` で読む (§1・§3) | **キャッシュ専用の読み口** (`ohlcv_cache` 対象) で読む。履歴テーブルには到達しない |
...
**実施順序が変わった**: 本書の task 分割案 (§後半) は「窓計算 → 配線 → `_cached_bars` 適用 → E2E」だったが、**分割 (プラン 9 Task 16) を `lookback_days` 配線より前に置く** — 旧テーブル前提で実装・テストしてから差し替えると、API・fixture・SQL・assert を作り直すことになるため。正しい順序はプラン 9 設計書 §2 を正とする。
...
#### **`lookback_days` を `_cached_bars` まで運ぶこと (必須 — 改訂 2 が落とした)**

現行の `_cached_bars(pair, interval, now, errors)` (`:192`) は **`lookback_days` を
受け取っていない**。`get_bars` (`:139`) は持っているので、**シグネチャに必須引数として
足し、呼び出し (`:182`) でそのまま渡す**。

既定値で誤魔化してはならない — `latest_1m_bar` は明示的に `lookback_days=1` を使う
(`:353-356`) 一方、通常の既定は 5 である。helper 側で推測すると両者を区別できない。
**helper の単体テストだけでは配線欠落を検出できない**ので、非既定の `lookback_days` を
`get_bars` から通して `load_bars(since=...)` まで到達することを見る検査点を置く (テスト 15)。

#### **窓計算は source 単位の `try` の中に置くこと (必須)**

`_finest_native_base` は、その source が interval を提供も導出もできなければ
`DataUnhealthy` を投げる (`:301-311`)。ライブ経路では source 1 件分の処理全体が `try` に
入っており、失敗すれば `errors` に記録して**次の source へ進む** (`:151-180`)。

**キャッシュ経路も同じ隔離にする。** 現行 `_cached_bars` の `try` は `validate_bars` の
直前にしかない (`:225-244`) ため、素直に窓計算を `for name` の冒頭へ置くと
**例外が `_cached_bars` 全体から漏れる**。先順位 source が導出不能というだけで、
後順位 source の健全なキャッシュを試さなくなる (= ライブ経路の優先順位契約と不一致)。

現行 3 source は全て `1m` を持つので直ちには発火しないが、**能力表を拡張した時点で
無音の全体障害になる**。テスト 16 で pin する。
...
2. **order の seed は不要** — **改訂 2 の記述は誤りだったので訂正する**。
   `_process_exits` の末尾に **`settings.pairs` 全件を走査して `bars_fn(pair)` を呼ぶ
   processed-bar marking** がある (`scheduler.py:969-977`)。コメントも
   「account の有無に関わらず (…) 必ず行う」と明記している。したがって
   **空の初期 DB でも tick が `_process_exits` に到達すれば 1m は取得・保存される**。

   **指揮者の実測が既にこれを示していた** — 空 DB で `tick()` を 3 回呼んで
   1m が 902 行書かれている (C3 調査)。自分の証拠を読めていなかった。
   ...
   **空の注文 DB** を使えば注文依存の手前処理はほぼ消える (これも order を seed
   ...
   壊れる。注文依存経路も pin したい場合は**追加条件**として分け、複数回呼び出しを
   許容する fixture にすること
3. ライブ成功フェーズでは **1m だけを返す**ようにし (複数回呼ばれても枯れない fixture)、
   書かれた source を確認する
...
| 16 | **先順位 source が導出不能でも、後順位 source の健全なキャッシュに進む** | 窓計算を source 単位の `try` の外に置く |

**変異は 1 つずつ独立に当て、殺した固有のテスト名を記録する。**

### 「一つの検査目的につき一テスト」と「一テスト一 assert」は別物 (codex M1)

短絡の穴 (プラン 8 Task 19/20 で 2 度実測) が言っているのは
...
2.5 **呼び出しごとの `lookback_days` が `get_bars` から `load_bars(since=...)` まで届く**
   (既定値の推測で代用していない)
2.6 **先順位 source の窓計算が失敗しても、後順位 source のキャッシュに進む**
