# [switch-ops-hardening] 設計書 v1.8c
...
| R9 | シェルの `approve <id>` の結果報告 (段 0 の設計判断 1) | **`approve_candidate` が返す `ApprovalOutcome` を文言に写す**。lock 解放後に DB を読み直す方式は廃止。文言生成は `approval retry` と共用する (`_retry_outcome_text` → `_approval_outcome_text` に改名・一般化)。**採用根拠**: §3.5 が retry について排したのと同じ欠陥クラスで、小さく、別束にするとレビュー材料を 2 回作ることになる (2026-09-20 ユーザー裁定)。→ §3.6 / AC-17a〜d |
| R10 | 巻き戻しの `conn.commit()` の位置 (段 0 の設計判断 2、S0-55) | **lock の内側**で完了させることを設計書に明文化する。**採用根拠**: lock を放した瞬間に他プロセスが未 commit の旧行を非終端として読む窓を防ぐ。**本体コードの変更は不要** (現実装の 5 箇所すべてが内側 — §3.3.2 の全数表)。**別コネクションから読む pin を足して S0-54 / S0-55 / S0-75 を殺す** (2026-09-20 ユーザー裁定)。→ §3.3.2 手順 4 / IV-6 / AC-9d |
| R11 | 再開時に `switch_required` が食い違う (2 周目 `/code-review` #1、probe で再現) | **案 3**: `approve_candidate` が自分の未完 journal (`switched` 未到達 = `preparing` / `versioned` / `recorded`) を再開するとき、**その回に再計算した `switch_required` が journal に保存された値と食い違ったら、その行を巻き戻して閉じ、新しい `op_id` で頭から流し直す** (0d-2c と同形)。activity に**専用イベントを 1 本**残す。**`_finalize_decision` の phase ガード (T2) は緩めない** — 緩めると本束以前の fail open (第三者が置いた symlink の中身を無検証で approved にする) に戻る。**採用根拠は §3.2「#### 再開の前提が崩れていたら…」** (2026-09-20 ユーザー裁定)。→ §3.2 / AC-16c-1〜6 |
| R12 | `afx plugin retire` の案内 (同 `/code-review` #5) | **同じ原因には同じ案内を出す**。`_plugin_retire` の `UnresolvedJournalError` にも bless (§3.4) と同じ「次の一手」(`approval list` → `approval retry`) を添える。→ §3.4 / AC-18 |
| R13 | 再開の前提の**判定述語を強める** (codex sol 設計レビュー r5 #2 = 指揮者の問い 4 への回答) | **3 つ組 `(switch_required, old_kind, old_target)` が保存値と一致するか**で判定する。1 つでも違えば巻き戻して新 `op_id`。**根拠**: journal の `old_target` は監査情報ではなく **`_revert_one` が実際に書き戻す補償データ**で、古いまま使うと**第三者の変更を上書きして戻す**。実装差は比較式 1 つ。codex は「強い述語で新たに壊れる正常系は見当たらない」と判定。**v1.7 の ticket 候補 `[resume-stale-old-target-not-detected]` は起票せず本束で閉じる** (2026-09-20 ユーザー裁定)。→ §3.2 / AC-16c-1〜7 |
| R14 | **版 dir の hash 再照合を常に通す** (同 r5 #3 = 指揮者の問い 6 への回答) | **切替の要否にかかわらず、`approved` にする前に `new_target` の版 dir の中身を候補の `(content_hash, artifact_hash)` と照合する**。不一致なら承認を成立させず `pending` 留置。**根拠**: T11 は現行 HEAD では `ValueError` で**到達不能**だった「第三者が live を新版に向けた → 無検証 approved」を**到達可能にする** (codex r5 #3 — T11 前は fail closed、T11 後は fail open)。照合は安く、「第三者が置いた中身を無検証で approved にしない」は本束の主旨そのもの。**ticket `[switch-not-required-skips-hash-verify]` を本束で閉じる** (2026-09-20 ユーザー裁定)。**v1.8b 訂正**: 「無検証で approved」は過大 — `history_git.record_version` が switch_required に関係なく毎回版 dir の実ファイルを独立に再照合しており (§3.7.1)、R14 が実際に塞ぐのは `record_version` の照合後〜`_finalize_decision` 前の TOCTOU の窓。多層防御として T13 の実装は変えない。→ §3.7 / IV-7 / AC-19a〜d |

**番号の付け方について (v1.8 注)**: 指揮者の依頼文では本 2 件を「R12 / R13」と呼んでいるが、
**v1.7 で R12 は既に `retire` の案内に割り当て済み**で、振り直すと §3.4 / AC-18 の参照が壊れる。
そこで**本書では R13 / R14 とする** (内容は依頼文の R12 / R13 と同一)。
[[spec-plan-with-changelog]] の「ID は振り直さない」規律に従った。
...
- `backtest/cli.py`: `_plugin_bless` / `_plugin_materialize` の例外処理を揃える。
  **v1.7 で `_plugin_retire` を追加** — 既存の `except (UnresolvedJournalError, ValueError, OSError)` を
  **2 節に割り、`UnresolvedJournalError` にだけ「次の 1 手」を添える** (R12、§3.4)。
  **捕まえる例外の集合は 1 つも変わらない** (節を割るだけ) ので、新しい握りつぶしは生じない。
- `switch.py`: **再開の前提 (`switch_required`) が崩れていたら巻き戻して新しい `op_id` で流し直す**
  (R11 + **R13**、§3.2)。`_close_own_unfinished_journal_if_any` を再利用し、**新しい `return` 地点も
  新しい `_revert_one` 呼び出し元も増やさない**。
- `switch.py`: **版 dir の hash 再照合を切替の要否にかかわらず通す** (R14、§3.7)。
  **`_advance_to_decided` の戻り値が `bool` になり、`approve_candidate` に `return` 地点が 1 つ増える**
  (既存の `still_pending(reverify_failed)` に合流するので文言表は不変。§3.5 の `return` 地点表に追記済)。
  **`_revert_one` の呼び出し元は 5 箇所のまま** (閉じるのは helper 経由)。
- `switch.py`: 2 段 phase ガード (r1 Important 2 / r2 Important 1 の実測から追加採用、§3.2) と、lock 内 outcome の戻り値 (r2 Important 4、§3.5)。
- `commands.py`: `approval list` 新設、`approval retry` の結果報告を **lock 内 outcome から写す**、
  **`approve <id>` (kind=plugin) の結果報告も同じ outcome から写す** (v1.6、§3.6)、`_HELP` 更新。
- **既存テスト 4 本**の書き換え (§7.1、v1.5 で 3 → 4。**v1.6 の (A) は本数を増やさない** —
  `tests/test_commands.py::test_approve_plugin_kind_reports_actual_outcome_not_always_approved` が
  無改変で緑になる文言形を選んだ、§3.6)、runbook (`docs/operations/indicator-initial-set-deploy-2026-09-19.md`) の改訂。
...
#### 再開の前提が崩れていたら巻き戻して新しい `op_id` で流し直す (v1.7、R11 = 2 周目 `/code-review` #1)

**出所**: 2 周目 `/code-review high` の指摘 #1。**probe で再現済み** —
`tmp/review-20260920-soh/r2/cr1-probe.md` (経路の file:line と実測出力)、
probe 本体 `tmp/review-20260920-soh/r2/cr1-probe/test_probe_cr1.py` (`4 passed`)。

##### 何が起きるか (動作目線)

配備の途中でプロセスが落ち、journal が `preparing` / `versioned` / `recorded` のどれかで止まる。
その後、**第三者 (または人間) が live を手で動かす**。人間が `approval retry <id>` (または `approve <id>`) を打つ。

- 0d は「自分の未完行だ」と判断して **`op_id` を再利用する** (`switch.py:1567-1568`)。
  `phase == 'switched'` の分岐 (`:1569-1610`) には入らないので、**分類も巻き戻しも起きない**。
- 下流は live の現況から `old_kind` / `old_target` を読み直し (`:1692-1700`)、
  **`switch_required` を再計算する** (`:1719`)。
- しかし journal 行の `switch_required` は**作成時の値のまま**で、更新する API は store 層に存在しない
  (`store/plugin_switch_journal.py` は `insert` / `set_phase` / `set_temp_path` / `get` /
  `get_open_by_name` / `list_non_terminal` のみ)。
- 結果、**切替の要否を決める値・実際に切替をする判断・`_finalize_decision` の期待 phase の 3 者が
  別々の値を見る**。

| 変種 | 保存値 | 再計算値 | 起きること (probe 実測) |
|---|---|---|---|
| **(a)** | `switch_required=1` | `False` (第三者が live をちょうど `new_target` へ向けた) | `_advance_to_decided` が切替ブロックを丸ごとスキップし phase は `recorded` のまま → `_finalize_decision` が保存値から期待 `switched` を導き **`ValueError`**。`preparing` / `versioned` / `recorded` の 3 通りとも、以後**何回 retry しても同じ例外**。再起動時 reconcile は非 `switched` 行を飛ばし (`:271-277`)、`process_expired_approvals` は未完 journal があるとスキップするので、**無人の抜け道が 1 つも無い** |
| **(c)** | `switch_required=0` | `True` (第三者が live を消した / 別の先へ向けた) | `advance_switch_journal` の `switch_required=0` ガード (`:150-153`) が先に落とす。**FS は触られない** (fail closed) が、やはり永久に詰まる。**本束以前からの既存欠陥** |

どちらも症状は「この approval が詰まる」ではなく **「この plugin 名が丸ごと凍る」** —
非終端行が残るので、別 approval の `approve_candidate` (`:1611-1631`) も `bless_candidate` (`:1985-1991`) も
`UnresolvedJournalError` で拒否される。抜け道は **`reject` ただ 1 つ**で、しかも `_revert_one` は
非 `switched` 行に FS 副作用を持たないため、**却下したのに live は第三者の状態のまま残る**。
人間に見えるのは内部用語の 1 行 (`op_id=1: _finalize_decision expects phase='switched' but found 'recorded'`)
と `_HELP` 全文だけで、**activity には痕跡が 1 行も残らない**。

##### 採った形 = 案 3 (検討して退けた案も記す)

| 案 | 動作 | 採否と理由 |
|---|---|---|
| 案 1 | `_advance_to_decided` が**その回の `switch_required`** を `_finalize_decision` へ渡し、期待 phase をそれで決める | **不採用**。(i) **(c) を閉じない** — (c) は `advance_switch_journal` の `:150` ガードで先に落ちるので、別途 `switch_required` の書き換えが要る。(ii) `switch_required=False` 枝は `_version_dir_hashes_ok` を通らない (`:1334` の `if switch_required:` の内側にしかない) ため、**第三者が置いた symlink の指す版を無検証で approved にする** = 本束以前の fail open に戻る。塞ぐには hash 再照合の追加が必須で、差分は最小でなくなる。(iii) journal 行の `old_kind` / `old_target` は作成時のまま残るので、行が「この操作が何をしたか」を語らなくなる |
| 案 2 | 再開時に保存 `switch_required` を再計算値で UPDATE する | **不採用**。`_revert_one` は `old_kind` / `old_target` で巻き戻す (`:182-195`)。`switch_required` だけ書き換えると、後の巻き戻しが **resume 時点ではなく作成時点の `old_target`** へ戻す。整合させるには 4 列全部を書き換える = **行の同一性を作り直す**ことになり、案 3 と同義でありながら監査記録を上書きで失う。store 層に新しい UPDATE API も要る |
| **案 3 (採用)** | 再計算値と保存値が食い違ったら、**停止行を巻き戻して閉じ、`op_id = None` にして新しい行で頭から流す** | **採用**。(1) **既存の 0d-2c (T3) と完全に同形** — 「再開できない停止行は巻き戻して新しい行で頭から流す」という判定軸 (「保存された前提が今も成り立つか」) の別条件を同じ処理へ合流させるだけで、**新しい概念を 1 つも持ち込まない**。(2) **(a) と (c) を 1 つの是正で両方閉じる**。(3) journal 行を書き換えないので `_revert_one` が使う `old_kind` / `old_target` が常に「その行が実際に出発した状態」を指す。(4) **IV-3 を構造的に守る** — 切替の要否・実際の切替・finalize の期待値が**同じ行の同じ値**に一本化される (今の欠陥は「3 者が別の値を見ている」こと自体)。(5) **監査行が 2 本残る** (停止行 `reverted` + 新行 `decided`) — §3.2 が `op_id = None` を必須にした理由と完全に整合 |

**`_finalize_decision` の phase ガード (T2) は緩めない。** 緩めれば (a) は通るが、
それは**本束以前の fail open** (第三者が置いた symlink を無検証で approved に戻す) そのものである
(§8.5 / `cr1-probe.md` §4 に旧コードの読解を記録)。

##### 判定を置く位置 — 0d の全分岐を数えて決めた

食い違いの判定には `old_kind` / `old_target` が要るので、**live の形を lock 内で読む `:1692-1700` より後**
にしか置けない。そこで **`approve_candidate` の `return` 地点を全数**列挙し、
**「食い違った未完行を抱えたまま return する経路」が残らないか**を確かめた。

| # | 分岐 / `return` 地点 | 自分の未完行 (`op_id`) を持っているか | 現行で行を閉じるか | v1.7 の扱い |
|---|---|---|---|---|
| 1 | `:1518-1522` `status != "pending"` → `already_decided` | **持たない** (0d より前。`op_id` は未設定) | — | **不変**。0d に到達しないので食い違いは生じない。なお「決着済みなのに非終端行が残る」状態は本束以前からの残余で、`reject` / `retry` の経路でしか解けない (§5 の残余に記録) |
| 2 | `:1526-1540` `_is_superseded` → `invalidated` | 同上 | — | **不変** (同上) |
| 3 | 0d: `existing_journal is None` | 持たない | — | **不変** (素の新規経路) |
| 4 | 0d: 別 `approval_id` の未完行 → `UnresolvedJournalError` | 他人の行 | — | **不変** (fail closed) |
| 5 | 0d-2b: 自分の `switched` 行 + `foreign` → `foreign_waiting` | 持つ (`switched`) | **閉じない (意図的)** | **不変**。`switched` 行なので `switch_required=1` が保存時も再開時も自明に成立しており、本節の食い違いは起きない。第三者の live は**触らないのが正** |
| 6 | 0d-2a: 自分の `switched` 行 + `switched` → `deployed` / `still_pending(reverify_failed)` | 持つ | `_reverify_switched_journal` が失敗時に閉じる | **不変** |
| 7 | 0d-2c: 自分の `switched` 行 + `not_switched` | 持つ | **閉じて `op_id = None`** | **不変** (v1.7 の判定はこの後段に置くので二重には走らない — `op_id is None` で素通り) |
| 8 | **0d: 自分の非 `switched` 行 (`preparing` / `versioned` / `recorded`)** | **持つ** | **閉じない** | ← **欠陥の入口**。以下 9〜12 で決着する |
| 9 | `:1657` 付近 `CandidateMissingError` → `still_pending` | 持ちうる | **閉じる** (`_close_own_unfinished_journal_if_any`) | **不変** (既に閉じている) |
| 10 | `:1667` 付近 `ValueError` → `still_pending(snapshot_invalid)` | 持ちうる | **閉じる** | **不変** |
| 11 | `:1683-1690` hash 不一致 → `still_pending(hash_mismatch)` | 持ちうる | **閉じる** | **不変** |
| 12 | **`:1702-1717` `old_kind == "plain"` → `legacy_plain_present`** | 持ちうる | **閉じない** ← **第 2 の穴** | **直る (v1.8: 判定を 1 箇所に統合したので、この分岐の手前で自動的に閉じる)**。下記 |
| 13 | `:1719` 以降の本線 (`begin_switch_journal` → `_advance_to_decided` → `deployed*`) | 持ちうる | — | **直す (T11)**。下記 |
| 14 | 例外の送出 (`UnresolvedJournalError` / `RuntimeError` / `OSError` / pin 解決の `ValueError`) | 持ちうる | 閉じない (意図的) | **不変**。§3.5 の送出契約どおり上へ抜け、行は非終端で残る → 次回 reconcile / retry が再評価する (fail closed) |

→ 12 と 13 以外に「食い違った未完行を抱えたまま return する経路」は無い。

##### 判定述語 = 3 つ組の一致 (v1.8 / R13) — **これにより判定は 1 箇所で足りる**

**v1.7 は `switch_required` の一致だけを見ており、判定を 2 箇所 (plain 分岐の手前 = T11-A と
`:1719` の直後 = T11-B) に置く必要があった。v1.8 で述語を 3 つ組に強めた結果、
判定は 1 箇所に畳める。**

```
保存値 = (journal 行の switch_required, old_kind, old_target)
再計算 = (この回の switch_required, old_kind, old_target)
1 つでも違えば「再開の前提が崩れている」
```

- **なぜ強める必要があるか (R13 の根拠)**: `old_target` は監査情報ではなく、
  **`_revert_one` が `_atomic_symlink_swap(..., target=row["old_target"])` で実際に書き戻す補償データ**
  (`switch.py:182-195`)。保存 `(symlink, X)` の行を、live が `Y` (≠ X、≠ `new_target`) になった後に
  再利用すると、**後の巻き戻しが第三者の `Y` ではなく古い `X` を復元する** =
  「第三者の変更を上書きして戻す」。`switch_required` だけの比較ではこの drift を検出できない
  (どちらも `1` のままなので)。
- **`switch_required` の一致は 3 つ組から導かれる** — `switch_required` は
  `not (old_kind == "symlink" and old_target == new_target)` (`:1719`) なので、
  `(old_kind, old_target)` が一致し `new_target` が同じなら自動的に一致する。
  **それでも 3 つ組で比べる** — `new_target` は `artifact_hash` 由来で、
  候補が差し替わっていれば `:1683-1690` の hash 照合が先に落とすので理論上は冗長だが、
  **保存値をそのまま比べる方が「行の前提が今も成り立つか」という意味に忠実**で、
  読む側が導出を追わなくて済む。
- **plain が自然に「不一致」になる**: journal 行の `old_kind` は DDL の CHECK 制約上
  `absent` / `symlink` の 2 値しか取れない (`db.py:169` — plain は行を作らない)。
  したがって **再計算が `plain` なら 3 つ組は必ず不一致**になる。
  → **v1.7 の T11-A (plain 分岐専用の `_close_own_unfinished_journal_if_any()` 呼び出し) は不要**。
  判定を **plain 分岐より前**に置けば、plain のケースは「前提が崩れた」として同じ処理で閉じられ、
  そのあと `legacy_plain_present` が**非終端行ゼロの状態で**返る。
  **結論: T11-A は T11-B に畳んだ** (専用分岐を 1 つ減らした)。

**`old_target` の保存形式と正規化の要否 (現物で確認した)**:

| | 場所 | 形式 |
|---|---|---|
| 書く側 (approve) | `switch.py:1692-1700` | `old_target = os.readlink(live)` の**生文字列** (相対、例 `.versions/sma/<64 hex>`)。symlink でなければ `None` |
| 書く側 (bless) | `:2005-2011` | 同じ式 |
| 保存 | `begin_switch_journal` → `journal_store.insert` | **加工せずそのまま** TEXT 列へ (NULL 可) |
| 使う側 | `_revert_one` (`:182-195`) | `_atomic_symlink_swap(..., target=row["old_target"])` で**そのまま symlink の中身**にする |
| 再計算側 (本判定) | 同じ `:1692-1700` の式 | **同一の式**なので like-for-like |

→ **正規化は不要**、かつ**してはいけない**。比較の両辺は**同じ `os.readlink` の生文字列**であり、
`resolve()` や `Path.as_posix()` を挟むと (i) dangling symlink で壊れ、
(ii) `./` の綴りを畳んで「見かけ上一致するが実体が違う」を作る (§3.1 と AC-5 が同じ結論)。
`None` は Python の `None` として往復する (`sqlite3` の NULL ↔ `None`) ので、
`None == None` / `None != ".versions/..."` が期待どおり効く。
**`classify_live` が使う `live.readlink().as_posix()` とは式が違う**が、
あちらは `new_target` / `old_target` との比較用で本判定とは別経路であり、揃える必要はない
(揃えると AC-5 の変異 killer が壊れる)。
...
if op_id is not None and (
        ...
        existing_journal["old_target"]) != (switch_required, old_kind, old_target):
    activity に switch_resume_precondition_changed を 1 行
    closed = _close_own_unfinished_journal_if_any()   # 巻き戻して commit
    if closed is not None:
        rolled_back_op_id = closed
        op_id = None

if old_kind == "plain":            # 以降は現行のまま
    ...
if op_id is None:
    op_id = begin_switch_journal(...)
```

- **activity は helper 呼び出しの直前に書く** — ログが因果の順に読める
  (`switch_resume_precondition_changed` → `_revert_one` が書く `switch_reverted`)。
  イベント名は**新規** (`grep -rn "resume_precondition" src/ tests/` は空)。
  フィールドは (**v1.8b で実フィールドに訂正**、`switch.py:1731-1738`)
  `name=<name> op_id=<M> phase=<p> stored=(<sw>,<old_kind>,<old_target>) now=(<sw>,<old_kind>,<old_target>) stored_switch_required=<0|1> recomputed=<0|1>`。
- `closed is None` は**到達不能** (helper が `None` を返すのは `op_id is None` か行が `switched` の
  ときだけで、判定の地点ではどちらも成立しない)。万一そうなっても `op_id` を保持したまま下流へ進み、
  **§3.2 の 2 段ガードが FS より前に `ValueError` で止める** ので fail closed。新しい分岐は足さない。

##### `_close_own_unfinished_journal_if_any` を再利用する (1 本化)

同 helper (`switch.py:1632-1649`) は既に「自分の未完行を `_revert_one` で閉じて commit する」処理そのもの
であり、**非 `switched` 行に対して FS 副作用を持たない** (`:182-195`) ことも本節の要件と一致する。
そこで **新しい巻き戻し関数は作らず、helper の戻り値だけを `None` → `int | None`
(閉じた `op_id`、閉じなければ `None`) に変える**。呼び出し元 9〜11 は戻り値を使わないので**不変**。

**この形を採る具体的な payoff**:
- **`approve_candidate` に新しい `return` 地点が 1 つも増えない** → §3.5 の `return` 地点表と
  **AC-14d (網羅性) はそのまま成立する**。
- **`_revert_one` の呼び出し元が増えない** → §3.3.2 の「全 5 箇所が lock 内 commit」の全数表が
  **そのまま真であり続ける** (IV-6 / AC-9d を作り直さなくてよい)。
- 巻き戻しは `_plugin_locks` (`:1516`) の内側で起き、helper が `conn.commit()` まで済ませるので
  **IV-6 (commit も lock 内) を自動的に満たす**。
- **AC-14d の但し書きを 1 文だけ改める** — 「`_close_own_unfinished_journal_if_any` は `-> None` の
  手続きなので bare `return` を持ってよい」は v1.7 で `-> int | None` になるため、
  「**入れ子関数なので `_walk_own_body` の走査対象外**」という本来の理由に書き直す (AC-14d 参照)。

##### 巻き戻した後の outcome — 既存の `deployed_after_rollback` に乗せる

**新しい outcome も新しい `reason` も作らない。** T11 の判定は 0d-2c と同じく
`rolled_back_op_id` を立てて `op_id = None` にするだけなので、末尾の組み立て (`:1737-1742`) が
自動的に `deployed_after_rollback` を返す。**`commands.py` の `_approval_outcome_text` は 1 行も変わらず、
§3.5 の文言表・§3.6.3 の `approve` 表・AC-14a / AC-17a も変わらない。**

**文言が事実と合うかを確認した** (変種 (a) は「切替をしていないのに `配備まで完了しました`」に見えるため):

| 文言の断片 | (a) での事実 | 判定 |
|---|---|---|
| `中断していた切替 (op_id=<M>) を巻き戻してから` | 停止行 `M` を `reverted` で閉じた。**FS の巻き戻しは no-op** (非 `switched` 行) | **可**。「切替」は journal 上の操作を指しており、symlink を動かしたとは言っていない |
| `再実行し` | 新しい `op_id` で版作成 → git 記録 → decide を頭から流した | **可** |
| `配備まで完了しました (plugins/<name> → <target>)` | live は `<target>` (= `new_target`) を指しており、approval は `approved` | **可**。**IV-3 の定義そのもの** (「approved になる瞬間に live が `new_target` を指している」)。切替を**行った**とは言っていない |

→ 新 outcome を足すより**既存の 1 語に合流させる方が、文言表・`_approval_outcome_text`・
AC-14a / AC-17a への影響が 0 で済む**。**採用**。

##### 起動時 reconcile は触らない (推奨)

reconcile は非 `switched` 行を飛ばす (`:271-277`)。したがって案 3 は
**`approve` / `retry` / (人間の) 契機でしか解けず、人間が retry を打つまで plugin 名は凍ったまま**になる。
**これを許容する。**

- **触らない理由**: (i) reconcile は**無人**の経路であり、非 `switched` 行は「別プロセスの
  `approve_candidate` / `bless_candidate` が今まさに進めている途中」でもありうる (FS 効果がまだ無いので
  reconcile が飛ばすのは設計どおり)。ここに自動巻き戻しを足すと、**進行中の操作を横から畳む**新しい競合を作る。
  (ii) 巻き戻しても approval は `pending` のままなので、**人間の操作は結局必要**であり、
  自動化の利得は「名前の凍結が早く解ける」だけ。(iii) IV-4 (承認の自動化禁止) 自体は破らないが、
  **無人経路に新しい FS 書込契機を足すのは本束の非スコープ** (「新しい自動化は作らない」)。
- **代わりに運用で閉じる**: v1.7 の後は **`approval retry` が (a) (c) のどちらも 1 回で解く**ので、
  凍結の解除手順は既存の「`approval list` → `approval retry <id>`」で足りる。
  **runbook にこの 1 行を足す** (§7.2)。`afx plugin bless` / `afx plugin retire` の
  `UnresolvedJournalError` にも同じ案内が出る (§3.4、R12)。

##### `bless_candidate` には波及しない (実コードで確認した)

`bless_candidate` は **未完 journal があれば無条件に `UnresolvedJournalError` を投げる**
(`switch.py:1985-1991`) — **再開経路を持たない**。`begin_switch_journal` (`:2066-2071`) は毎回
新しい行を、その場で読んだ `old_kind` / `old_target` (`:2005-2011`) から計算した
`switch_required` (`:2063`) で作るので、**保存値と再計算値が食い違う余地が構造的に無い**。
→ **`bless_candidate` は 1 行も変えない。** 共通関数化もしない (共有すべき処理が無い)。
bless から見える影響は「凍った名前が retry で解けるようになる」ことだけである。

##### v1.7 で「既知の残余」としていた 2 件は、v1.8 で**どちらも本束で閉じる**

| v1.7 の残余 | v1.8 の処置 |
|---|---|
| 1. **`switch_required=0` の枝は版 dir の hash 再照合を通らない** (`_version_dir_hashes_ok` が `_advance_to_decided` の `if switch_required:` の内側にしかない `:1334-1358`)。変種 (a) の再実行は `switch_required=False` で `approved` に至るので、**第三者が live に向けた版の中身が検証されない** | **R14 で閉じる (§3.7)**。ticket `[switch-not-required-skips-hash-verify]` は**起票しない**。v1.7 は「(a) に限らず既存の穴に戻るだけ」と評価したが、**codex r5 #3 がこの評価の不正確さを指摘した** — **現行 HEAD (T11 前) は変種 (a) を `ValueError` で完全に遮断している (到達不能)** のに対し、**T11 は同じ変種を到達可能な `approved` 経路に変える**。比較対象を「本束以前」ではなく「現行 HEAD」に取れば、T11 単独は fail open の**再導入**である。よって T11 と同時に閉じる (2026-09-20 ユーザー裁定) |
| 2. **`(old_kind, old_target)` が動いたのに `switch_required` が動かない drift を検出しない** | **R13 で閉じる** (判定述語を 3 つ組に強めた、上記)。ticket `[resume-stale-old-target-not-detected]` は**起票しない** |

**本束を通しても残る残余は §5 に一覧する** (reject の巻き戻しが live を分類しない / 決着済み approval に
非終端行が残る場合の凍結 / bless が照合不成立のとき自分の行を閉じない — §3.7)。
...
**T11 の判定を通る 1 回の retry も同じ表に載る**。判定は `S1` より後・`S3` より前の
**`R`** (= 停止行の巻き戻し + `reverted` の commit、FS 副作用なし) という 1 段が増えるだけで、
新しい行が作られた後は **`S3`〜`S7` が字句どおり同じ**である。

| 落ちた位置 (T11 の巻き戻し) | 残る状態 | 次の起動時 reconcile が見るもの | 次の `approval retry` が見るもの |
|---|---|---|---|
| `R` の前 (判定はしたが巻き戻す前) | 停止行 `preparing`/`versioned`/`recorded` / live は第三者の状態 / pending | `phase != 'switched'` → **skip** | 同じ判定をやり直す (**冪等** — 再計算値は同じなので再び食い違い、巻き戻しから流す) |
| `R` の後・`S3` の前 | 停止行 `reverted` (終端) / **非終端行なし** / pending | 非終端行が無いので何もしない | `get_open_by_name` が `None` → **素の新規 approve 経路**で完遂 (S2 の後と同じ行) |
| `S3` 以降 | 上表の `S3`〜`S7` と同一 | 同上 | 同上 |
...
4. 2 と 3 が両方とも一致したときだけ `_revert_one` (または該当の FS/DB 更新) を実行する。
   **`conn.commit()` も同じ lock の内側で完了させる** (v1.6、2026-09-20 裁定)。
   **理由**: `_revert_one` は `set_phase(..., commit=False)` で終わる (B-2 の層規律) ので、
   commit は呼び出し元の責務である。commit を lock の外へ出すと、
   **lock を放した瞬間から commit までの間、他プロセスが同じ lock を取って journal を読み、
   既に巻き戻した「はずの」行を非終端のまま見る窓**ができる。**lock の意味は「FS と journal の
   一貫した更新を、他者から中途半端な形で観測されずに完了させること」であり、
   commit まで含めて初めて成立する。**

   **実害の正直な見積り (v1.6、過大に言わない)**: SQLite は WAL + 単一ライタなので、
   この窓に入った別プロセスが古い `switched` 行を読んで `set_phase` を試みても、
   こちらの書込 tx が commit するまで `busy_timeout` で待たされ、待った先の UPDATE は冪等である。
   したがって**実害は IV-3 の破れ (approved なのに未配備) ではなく、
   「stale read に基づく二重の `_revert_one` / `switch_reverted` 監査行の重複」と
   最大 `busy_timeout` 分の停止**であり、**AC-9b-iii と同じ階級**である。
   加えて**クラッシュ耐性**: 巻き戻しと次の書込の間で落ちたときに巻き戻しが失われない
   (次回 reconcile が再評価するので収束はするが、**監査記録としての `reverted` 行が残らない**)。
   この 2 点をもって「lock 内で commit まで完了させる」を要件とする。

**この「読み直して一致を確認する」までが R5 の要件**であり、lock は前提条件にすぎない。

**この規律が既に成立していることの全数確認 (v1.6、`grep -n "_revert_one(" src/agentic_fx/plugin/switch.py`
の全 5 箇所)**: 本体コードの変更は**不要**である。
...
→ **v1.6 が足すのは pin (テスト) だけで、`switch.py` は 1 行も変えない** (AC-9d)。
...
| `_plugin_retire` (`cli.py:628`) | `except (UnresolvedJournalError, ValueError, OSError)` の **1 節で `エラー: {e}` だけ** | **v1.7 (R12)**: `UnresolvedJournalError` を**先行する別の節に割り**、bless と**同じ「次の 1 手」**を添える。`(ValueError, OSError)` の節は現行のまま残す。**捕まえる例外の集合も rc も変わらない** (節を割るだけ = 新しい握りつぶしを作らない) |
| `_plugin_submit` / `_plugin_lock` | — | **変更なし** (全数確認済、同型の穴なし。`submit_candidate` は journal を見ないので `UnresolvedJournalError` を投げない) |
...
**文言** (retire、v1.7 / R12 — bless と**同じ 2 行目**を使う):
```
エラー: plugin 'sma' has an unresolved switch journal (op_id=3) — resolve it first (reconcile or approval retry)
  収束手順: サービスの対話シェルで `approval list` → `approval retry <approval_id>`
```
**`retire_plugin` が投げるメッセージには `approval_id=` が含まれない** (`switch.py:1795-1798` —
`op_id=` のみ。`approve_candidate` / `bless_candidate` の文言とはここが違う)。
**だからこそ案内の第一手が `approval list` であることに意味がある** — 一覧の未終端 journal の節が
`op_id` / `name` / `phase` / `approval_id` を並べる (§3.5) ので、**この 2 行だけで人間は次に打つ id に辿り着ける**。
`retire_plugin` 側の例外文言は**変えない** (既存テスト `tests/plugin/test_materialize_retire.py:62` が
型だけを pin しており、文言を足す必要は無い)。
...
| `rolled_back_op_id` | 0d-2c **または T11 の判定 (v1.7 / v1.8、§3.2)** で畳んだ**停止行**の `op_id` (それ以外は `None`)。**`deployed_after_rollback` はこの値の有無で決まる** — 契機が 2 つになっても組み立ての 1 箇所は変わらない |
...
**v1.7 で同じ変数を立てる契機が 2 つになった** (0d-2c = `switched` 行の `not_switched` /
T11 = 非 `switched` 行の前提崩れ) が、**組み立ても文言も `_approval_outcome_text` も 1 行も変わらない**
(§3.2「巻き戻した後の outcome」で文言が事実と合うことを断片ごとに確認済)。
...
| **`_advance_to_decided` が `False` を返す (v1.8、R14)** | `switch_required=0` で版 dir の照合が不成立 (§3.7) | `still_pending` (`reason=reverify_failed`) — 呼び出し元が `_close_own_unfinished_journal_if_any()` で自分の行を閉じてから返す。**既存の `reverify_failed` に合流するので文言表は変わらない** (AC-19a が通す) |
...
**例外はこの表の外 — どの `except` に落ちてどう表示されるか (r4 I3、実コードで確認した)**
...
上記のいずれも `approve_candidate` で捕まえて `still_pending` に化けさせ**ない**。
理由: (i) 既存テストが `pytest.raises(RuntimeError, match="content_hash mismatch")` 等で
この送出を pin している、(ii) I/O 障害を「pending 留置」と同じ籠に入れると原因が消える。
**AC-7 (恒久失敗での連打) はこの送出経路で成立する** — DB/FS の終点 (旧行 `reverted` + 新行 `switched`) は同じ。

**戻り値の後方互換 (全数確認、v1.6 で 1 行訂正)**: `approve_candidate` / `retry_approval` は
**設計書 v1.2 で lock 内 outcome 方式に変えるまで** `None` を返していた。
呼び出し元は `commands.py` の approve ハンドラ / retry ハンドラ / reconcile (`switch.py`、`retry_approval` を呼ぶ) /
`retry_approval` 自身 (`approve_candidate` を呼ぶ) と、テスト 30 箇所あまり。
**v1.6 の時点で戻り値を使うのは retry ハンドラと approve ハンドラの 2 箇所**で、
reconcile と `retry_approval` の内部呼び出しは引き続き**戻り値を無視する**
(reconcile は無人経路なので報告先が無く、`retry_approval` は透過するだけ)。
テスト側は**すべて戻り値を使っていない文**なので、値を返すようにしても既存呼び出しは 1 つも壊れない。
...
**`None` / 未知 outcome の fallback は置かない (v1.4 で方針変更、r4 I2)**:
v1.3 は既存 spy との互換のために「`結果を判別できませんでした`」という fallback 行を置いていたが、
これは **本体の `return` 漏れ (outcome を返し忘れた return) を「普通の応答」に化けさせる**。
...
- そのため **`tests/test_commands.py:541` の spy は「実物より緩い fake」** であり、
  **実物と同じ形の outcome を返すよう書き換える** ([[test-fixtures-from-real-transcripts]] と同じ規律 —
  手書きの偽形状は緑のまま本番で外れる)。→ **書き換える既存テストは 4 本**になる (§7.1。
  r4 時点では 3 本と数えていたが、実装プランの着手前検証で `tests/plugin/test_reconcile.py::
  test_reconcile_resolution_holds_the_dependency_locks` も red になることが実測された — v1.5)。
- 返し忘れ自体を落とす受入条件を新設した (**AC-14d**)。
...
| (旧) 「approval 行が見つかりません」 | — | **文言ごと削除** (v1.3)。送出契約に統一 (上記) |
| (旧) `結果を判別できませんでした` | — | **文言ごと削除** (v1.4、r4 I2)。`None` / 未知は fail loud |

→ **文言表の全行が outcome のフィールド (と、シェルが自分で持つ approval id) だけで組み立てられることを確認した。
シェルは lock 外で DB も FS も読まない。** 7 種の outcome が 6 行の文言カテゴリに写る
(`already_decided` と `invalidated` は同じ 1 行に併記される)。
...
### 3.6 件 3 (続き): 対話シェルの `approve <id>` も lock 内 outcome を写す (v1.6、2026-09-20 裁定)

#### 3.6.1 何が起きているか (動作目線)

`afx>` で `approve 12` と打つ。plugin kind ならシェルは `approve_candidate` を呼ぶ。
**現行は、その戻り値 (lock の内側で確定した `ApprovalOutcome`) を捨てて、lock が解放された後に
`approval_requests` を `SELECT status, reason` で読み直し、その結果から文言を作っている**
(`commands.py:100-110`)。これは §3.5 が `approval retry` について「誤報を生む」として排した方式そのものである:

- `approve_candidate` が `_plugin_locks` を抜けた後、**別プロセスが同じ approval を正規に処理する**ことがある
  (期限切れ sweep、別プロセスの CLI `bless`、もう一人の運用者のシェル)。
  読み直した `status` はその別処理の結果であり、**自分が打った操作の結果ではない**。
- 最も分かりやすい誤報: 自分の approve は候補欠損で `pending` に留まったのに、
  読み直しの直前に別プロセスが候補を置いて承認を完遂すると、シェルは **`approval #12 approved` と表示し、
  さらに activity に `approved #12 via shell` と、自分が下していない決定を書く**。
- 逆向きの誤報も起こる: 自分の approve が `deployed` で完遂したのに、読み直しの直前に別プロセスが
  同じ approval を `invalidated` に落とすと、「approved になりませんでした」と表示される。

**本束が直している欠陥 (表示と現物のずれ) と同じ形**なので、隣に残さない。

#### 3.6.2 決め (retry と同じ規律)

- シェルは `approve_candidate` が返す `ApprovalOutcome` を**文言に写すだけ**にする。
  **lock の外で DB も FS も読み直さない。**
- **文言生成は `approval retry` と共用する。** `commands.py` の `_retry_outcome_text` を
  **`_approval_outcome_text` に改名して一般化する** (中身は 1 行も変えない — 7 outcome → 6 文言、
  未知 / `None` は `ValueError` で fail loud)。docstring の「T5」は「T5 / T9」に改める。
- 接頭辞だけをコマンドごとに分ける (retry = 「再試行しました」、approve = 下表)。
- **表示 id は `args[0]` (文字列) のまま**にする — `retry` は `approval_id` (int) を使っており不揃いだが、
  `approve 007` のような入力で表示が `#007` から `#7` に変わってしまうのを避けるため、本束では揃えない (2026-09-20 裁定)。

#### 3.6.3 `approve <id>` で人間に何が表示されるか (outcome 別、全 7 種)

`<text>` = `_approval_outcome_text(outcome)` の出力 (§3.5 の文言表と**同一文字列**)。

| `outcome` | 戻り文字列 (逐語) | activity `approved` を書くか |
|---|---|---|
| `deployed` | `approval #<id> approved: 配備まで完了しました (plugins/<name> → <target>)` | **書く** |
| `deployed_after_rollback` | `approval #<id> approved: 中断していた切替 (op_id=<rolled_back_op_id>) を巻き戻してから再実行し、配備まで完了しました (plugins/<name> → <target>)` | **書く** |
| `foreign_waiting` | `approval #<id> は今回の操作では承認されませんでした (status=pending): live が第三者に触られているため自動収束しません (op_id=<op_id>)。plugins/<name> の状態を確認してください` | 書かない |
| `still_pending` | `approval #<id> は今回の操作では承認されませんでした (status=pending): approved になりませんでした (reason=<reason>)` | 書かない |
| `legacy_plain_present` | `approval #<id> は今回の操作では承認されませんでした (status=pending): plugins/<name> が旧式のディレクトリのままです (先に afx plugin retire が要ります)` | 書かない |
| `already_decided` | `approval #<id> は今回の操作では承認されませんでした (status=<status>): この承認は既に決着しています (status=<status>)` | 書かない |
| `invalidated` | `approval #<id> は今回の操作では承認されませんでした (status=invalidated): この承認は既に決着しています (status=invalidated)` | 書かない |

- **接頭辞に `(status=...)` を置く理由**: 既存テスト
  `tests/test_commands.py::test_approve_plugin_kind_reports_actual_outcome_not_always_approved` (`:127-157`) が
  **`"pending" in out`** と **`out != f"approval #{id} approved"`** の 2 つを pin している。
  `status` を接頭辞に置くと**この既存テストが 1 行も書き換えずに緑**になり、
  かつ「今この approval はどの状態か」という運用上いちばん知りたい値が先頭に来る。
  接頭辞を「承認されませんでした」とし、`_approval_outcome_text` の文言 (`still_pending` の
  「approved になりませんでした」等) とは異なる語にしたため、**`still_pending` で文言が重複することは無い**。
  `already_decided` / `invalidated` では `(status=...)` が接頭辞と `<text>` に 1 回ずつ現れるが、
  **情報は減っていない** (前段は「今の状態」、後段は「決着済みである」の理由付け) ので許容する。
- **`deployed*` だけが「承認が成立した」である。** `already_decided` で `status == "approved"` の場合
  (= 既に approved 済みの approval をもう一度 approve した) も **`approved:` の形にはしない** —
  この呼び出しは決定を下していないからである。**現行からの意図的な挙動変更** (§3.6.5)。

#### 3.6.4 変わらないもの (全数確認した)
...
| `approval retry <id>` の文言 | §3.5 の表 | **不変** (接頭辞も文言も 1 文字も変えない) |

#### 3.6.5 現行から**変わる**もの (意図的、全数)

| # | ケース | 現行 | v1.6 | 判断 |
|---|---|---|---|---|
| 1 | 二重 approve (plugin kind、既に `approved`) | `approval #<id> approved` を返し、**activity に 2 本目の `approved` を書く** | `approval #<id> は今回の操作では承認されませんでした (status=approved): この承認は既に決着しています (status=approved)` / activity は書かない | **是正**。決定を下していない呼び出しが決定の記録を残すのは誤り。既存テストの pin は無い (`test_approve_already_decided` は非 plugin kind) |
| 2 | 失敗時の `reason=` の出所 | `approval_requests.reason` 列 (DB を読み直した値) | `still_pending` のときだけ `outcome.reason` (`candidate_missing` / `snapshot_invalid` / `hash_mismatch` / `reverify_failed`) | **是正**。旧表示の `reason` は **pending の行では常に `-`** で、実質「既に rejected / expired されていた場合の他人の理由」しか出していなかった。人間の却下理由の読み出し導線は `approval <id>` の詳細表示 (`_approval_detail` が `reason=` を出す、`commands.py`) が正であり、こちらは write-only にならない |
| 3 | 別プロセスの決定を自分の結果として報告する (§3.6.1) | 起こる | **起こらない** | 本束の主旨 |

#### 3.6.6 遮断・不変条件との関係

- **IV-4 (承認の自動化禁止) は不変** — 本節は表示と activity の話であり、承認の契機を増やさない。
- **遮断 8 は不変** — 新しい文字列を `approval_requests.reason` / `improvement_backlog.last_result` に書かない。
  `Commands.dispatch` の戻り値は端末に出るだけで改善プロンプトへ流れる経路を持たない (§3.5 の全数確認と同じ)。
- **IV-3 の検証は実装側の assert と AC の仕事** — 表示側は lock 内で確定した事実だけを写す (§3.5 と同じ)。

### 3.7 版 dir の hash 再照合を**切替の要否にかかわらず**通す (v1.8、R14)

**出所**: codex sol 設計レビュー r5 #3 (= 指揮者の問い 6 への回答)。ユーザー裁定 2026-09-20。

#### 3.7.1 何が起きているか (現物で確認した)

`_advance_to_decided` (`switch.py:1288-1360`) の流れ:

| 段 | 行 | `switch_required=1` | `switch_required=0` |
|---|---|---|---|
| 版作成 | `:1317-1322` | `create_version_dir` (候補の 3 ファイルから) | **同じく呼ぶ** |
| `versioned` | `:1323-1325` | advance | **同じ** |
| git 記録 → `recorded` | `:1327-1332` | advance | **同じ** |
| `switched` → `switch_live` → **再照合** | `:1334-1358` | `_version_dir_hashes_ok` で照合、**不一致なら `RuntimeError`** | **ブロックごとスキップ = 照合しない** |
| `_finalize_decision` | `:1359` | 到達 | 到達 |

**穴の機構は `create_version_dir` の冪等性にある** (`version_store.py:78-86` 逐語):

```python
final_dir = name_dir / artifact_hash
if final_dir.is_dir():
    return final_dir        # ← 既に在れば中身を作り直さない
```

→ **`switch_required=0` の枝では、版 dir は「この回に作ったもの」ではなく
「既に在ったもの」である**。第三者がその中身を in-place 編集していても、
`create_version_dir` は上書きせず、照合も走らないので、**中身が候補と違うまま `approved` になる**。

**v1.8b 訂正 (T13 実装時の実測、`tmp/wt/soh` HEAD `cd792f5` で確認)**:
上の表と「照合も走らないので中身が候補と違うまま approved になる」は**過大**だった。
実際には `history_git.record_version` (`history_git.py:135-138` / `:161-172`) が
`_advance_to_decided` の呼び出しのたびに**無条件で** (switch_required の値に関係なく、
上表の「git 記録 → `recorded`」の段で) 版 dir の実ファイルを読み直して git blob hash を取り、
渡された候補の `(content_hash, artifact_hash)` と独立に再照合し、不一致なら
`HistoryGitError("... index blob hash mismatch ...")` を送出する。
これは switch_required の分岐に入る**前** (`:1327` は `:1334` の `if switch_required:` の外) に
毎回走るので、「`switch_required=0` では版 dir の中身が一切検証されない」という上の記述は誤り。
**R14 (本節の照合、`_version_dir_hashes_ok`) が実際に塞ぐのは、
`record_version` の照合が通った後〜`_finalize_decision` の前に版 dir が書き換わる
TOCTOU の窓だけ**である (`record_version` の前に改竄されていれば `HistoryGitError` が先に
上がり、本節の照合には到達しない)。この訂正は R14 の必要性そのものは変えない —
TOCTOU の窓が実在する限り「無検証で approved になる」経路は残るため、
**T13 の実装は多層防御としてそのまま残す**。上表・「穴の機構」の説明・§0 R14・§5.2 の
「無検証」表現は、この意味で読み替えること。
`tests/plugin/test_switch_ops_hardening.py` の T13 テストが `_record_then_tamper_version_dir`
(= `record_version` を実体で呼んだ**直後**に版 dir を改竄する seam) で構成されている理由も
これに尽きる — 素朴に `create_version_dir` の直後で改竄すると `record_version` の照合が
先に `HistoryGitError` を投げてしまい、T13 が新設する分岐を検証できない。

**v1.7 の評価は不正確だった** — v1.7 は「(a) に限らず `switch_required=0` の正規経路すべてに
共通する既存の穴に戻るだけ」と書いたが、**変種 (a) に限っては違う**。
**現行 HEAD (T11 前) は変種 (a) を `_finalize_decision` の phase ガードで `ValueError` にしており、
そもそも `approved` に到達しない (fail closed)。T11 はこれを到達可能な `approved` 経路に変える。**
つまり **T11 単独では fail open の再導入**であり、T11 と同時に閉じる必要がある (codex r5 #3)。
...
2. **処置は「その行に自動収束経路があるか」で分かれる** (§0 R2 の「判定は 1 つの関数に寄せ、
   **処置は actor で分かれてよい**」と同じ形):
...
3. **修復 (候補からの版の作り直し) は行わない。** `_reverify_switched_journal` は
   `switched` 行に対して壊れた版を消して候補から再作成するが、**`switch_required=0` 枝では同じことをしない**。
   **理由**: (i) **ユーザー裁定の文言が「不一致なら承認を成立させず pending 留置」**であること (これが主たる根拠)、
   (ii) **来歴 (provenance) が違う**。`switched` 行は「**このシステムが切替の途中でその live を置いた**」
   ことの証拠であり、そこで版を作り直すのは**自分の操作を最後まで済ませる**ことにあたる。
   対して `switch_required=0` の枝の live は、**過去の正規の承認が置いたものかもしれないし、
   第三者が置いたものかもしれず、コードには区別がつかない**。ここで修復すると
   **第三者が置いた symlink を「承認済みの配備」に洗い替える**ことになる —
   これは R1 が `foreign` に対して定めた「何が正しいか機械には決められないので触らない」の一般形である。
   **収束経路はある** — 人間が壊れた版 dir を消して `approval retry` を打てば、
   `create_version_dir` が候補から作り直して成立する。**runbook に書く** (§7.2)。
4. **outcome は既存の `still_pending(reason="reverify_failed")` に合流する。**
   `reverify_failed` は 0d-2a の `_reverify_switched_journal` 失敗で既に使われており
   (§3.5 の `return` 地点表)、意味は同じ (「この approval が指す版が候補と一致しないので承認しなかった」)。
   → **outcome 列挙・文言表・`_approval_outcome_text`・AC-14a / AC-17a は 1 行も変わらない。**
5. **`_advance_to_decided` の戻り値を `None` → `bool` にする** (`True` = decided まで進んだ /
   `False` = 照合不成立で決定しなかった)。**既存の呼び出し元 3 箇所への影響を全数確認した**:

| 呼び出し元 | 現行 | v1.8 |
|---|---|---|
| `approve_candidate` (`:1728-1733`) | 戻り値を捨てる | **`False` なら `_close_own_unfinished_journal_if_any()` → `still_pending(reverify_failed)`** |
| `bless_candidate` (`:2080-2085`) | 戻り値を捨てる | **`False` なら `RuntimeError` を送出** (下記 3.7.3) |
| `tests/plugin/test_switch_ops_hardening.py:513` (AC-16a の入口ガード) | 戻り値を使わない | **無改変で緑** |

#### 3.7.3 `bless_candidate` への波及

bless も同じ `_advance_to_decided` を通るので、**照合は bless でも必ず走る** (これは望ましい)。
`switch_required=0` の bless = **同じ候補の再 bless で live が既にその版を指している**状態
(runbook §6.3 手順 5 が毎回通る経路)。

- **照合が通る通常系は不変** — 版 dir の中身が候補と一致していれば従来どおり `approved`。
  **既存テスト `tests/plugin/test_indicator_initial_set.py:866-870` (手順 5 の 2 回目の bless) は無改変で緑**
  (AC-19c がこれを明示的に観測する)。
- **照合が落ちたとき**: `RuntimeError` を送出し、CLI は rc=1 + `エラー: ` を出す (§3.4 の作法)。
  **bless は自分の未完行を閉じない**。
  **文言の決め (v1.8c、2 周目やり直し `/code-review high` r2b #3 の是正 = `b126dd2`)**:
  この分岐は `switch_required=0` (`_advance_to_decided` が `False` を返す枝) でのみ到達し、
  `switch_live` は一度も呼ばれていない。よって**「切替後」(after switch) とは言わない**
  (`switch_required=1` 側 `:1354-1357` の `RuntimeError` は文言もろとも既存 pin — 変更していない)。
  bless の文言は「版 dir の content_hash mismatch」とし、`content_hash mismatch` 部分文字列のみ
  既存 pin (AC-19d) を維持する。
  **理由**: bless には `_close_own_unfinished_journal_if_any` に相当する内部 helper が無く、
  閉じるために `_revert_one` を**6 箇所目**として足すと §3.3.2 の全数表と IV-6 / AC-9d を作り直すことになる。
  **到達の現実性**: bless の候補は `plugins/_human/<name>` で、bless はその場で読んで hash を取っている。
  版 dir が候補と食い違うのは**第三者が版 dir を in-place 編集した場合だけ**で、
  そのとき人間に必要なのは「勝手に直す」ことではなく「止まって知らせる」ことである。
  → **残余として §5 に明記**し、凍結の解除は R12 の案内 (`approval list` → `approval retry` / `reject`) と
  runbook (§7.2) で拾う。

#### 3.7.4 `_reverify_switched_journal` との関係 (重複しないか)

| | `_reverify_switched_journal` (`:1393-1481`) | 本節の照合 (`_advance_to_decided` 内) |
|---|---|---|
| 呼ばれる契機 | **0d-2a** — 既に `switched` の行を**再開**するとき | **その回に `recorded` / `switched` まで進めた**行を decide する直前 |
| 使う述語 | `_version_dir_hashes_ok` (**同じ関数**、確定-7 で共有済) | 同じ |
| 修復 | する (壊れた版を消して候補から再作成、activity `switch_reverify_version_mismatch`) | **しない** (3.7.2 の 3) |
| 失敗時 | `_revert_one` で閉じて `False` → `still_pending(reverify_failed)` | `switch_required=1` は `RuntimeError` / `switch_required=0` は `False` → 呼び出し元が閉じて `still_pending(reverify_failed)` |

→ **述語は共有済みで重複しない。契機が排他** (再開 vs この回の前進) なので、
**同じ呼び出しで 2 回照合が走ることはない**。0d-2a は `_advance_to_decided` を通らずに
直接 `_finalize_decision` を呼ぶ (`:1589-1598`) ためである。

#### 3.7.5 正常系が誤って落ちないこと

**照合が通る条件は「版 dir の 3 ファイルの hash と dir 名が候補の `(content_hash, artifact_hash)` と一致」**
(`_version_dir_hashes_ok` `:1362-1391`)。この回の `_advance_to_decided` は
**同じ `artifact_hash` で `create_version_dir` を呼んでいる**ので、
版 dir が無ければ候補から作られて必ず一致し、在れば (第三者が触っていない限り) 一致する。
→ **通常の「同じ候補の再承認 / 再 bless」は 1 件も落ちない。**
**AC-19c (正常系の回帰) と、既存テスト `test_runbook_post_gate_failure_converges_via_approval_retry` の
手順 5 が二重の網**になる。
...
| IV-7 | **(v1.8、R14)** approval が `approved` になる瞬間に、**`new_target` の版 dir の中身が候補の `(content_hash, artifact_hash)` と一致し、dir 名自体も `artifact_hash` と一致している** (`_version_dir_hashes_ok`)。**切替の要否にかかわらず成立する** — IV-3 が**パス名**の一致を要求するのに対し、IV-7 は**中身**の一致を要求する。v1.7 までは `switch_required=1` の枝でしか成立していなかった (§3.7) |
...
| `approve_candidate` の自分の未完 journal を閉じる `_revert_one` | `switch.py:1606` (0d-2c) / `:1647` (`_close_own_unfinished_journal_if_any`) | `_plugin_locks` の内側 (`:1516`) | はい (0d-2c は `get_open_by_name` `:1564` + `classify_live` `:1579`、`_close_own_unfinished_journal_if_any` は `journal_store.get` `:1645`) | 直さない (0d-2c の 1 本化先、§3.2)。**v1.7 / v1.8 の T11 の判定もこの `:1647` を再利用する**ので、`_revert_one` の呼び出し元は **5 箇所のまま増えない** (§3.3.2 の全数表と AC-9d はそのまま成立する) |
...
- **本束を通しても残る既知の残余 (v1.8 で更新)**:
  1. ~~`switch_required=0` の枝は版 dir の hash 再照合を通らない~~
     → **v1.8 の R14 (§3.7) で閉じた。** ticket `[switch-not-required-skips-hash-verify]` は起票しない。
  2. ~~`(old_kind, old_target)` の drift は検出しない~~
     → **v1.8 の R13 (§3.2) で閉じた。** ticket `[resume-stale-old-target-not-detected]` は起票しない。
  3. `reject_candidate` の巻き戻しは live を分類しない —
     ticket `[reject-revert-without-live-classification]` (IV-6 の例外 2、v1.6a)。**残る。**
  4. 決着済み (`already_decided` / `invalidated`) の approval に非終端 journal が残っていると
...
| **AC-5** | 変異: 分類器の比較を `readlink` から `resolve()` に変えると、**`plugins_root` の外を指す絶対パスで張った live symlink** のケースが red (raw では `foreign`、`resolve()` 化すると `switched` に化ける)。<br>**v1.5 訂正 (実装プランの着手前検証で実測)**: 当初書いていた「版 dir を消した dangling 状態」では red にならない — `Path.resolve()` は非 strict なので dangling でも同じ文字列を返し、`./` 付きの綴りも `as_posix()` が単一ドットを畳んでしまうため、raw と `resolve()` の分岐が割れない |
...
| **AC-9d** | **巻き戻しの commit が lock の内側で完了していること (v1.6、S0-54 / S0-55 / S0-75 の killer)**。(a) reconcile が `not_switched` 行を巻き戻した後、**同じ DB ファイルへの別コネクション**から読むと当該行が `reverted` である (S0-54: `_revert_under_lock` の `conn.commit()` を落とすと、ActivityLog はファイル出力で DB を触らず、`not_switched` 枝の後に commit する箇所も無いので未 commit のまま残り red)。(b) `_plugin_lock` の解放**直前**に別コネクションから読んだ時点で既に `reverted` である (S0-55: commit を `with` の外へ出すと、解放直前の観測では未 commit のまま = `switched` で red)。(c) `approval retry` が 0d-2c で巻き戻した後、候補欠損で `still_pending` を返して戻ってきた時点で、別コネクションから読むと停止行が `reverted` である (S0-75: 0d-2c の `conn.commit()` を落とすと、`CandidateMissingError` → `_close_own_unfinished_journal_if_any` は `op_id is None` で即 return し commit しないので、未 commit のまま red)。**本体コードの変更は伴わない — 現実装は 5 箇所すべて lock 内 commit であることを §3.3.2 で全数確認済で、AC-9d はそれを固定する pin である** |
...
| **AC-14a** | `approval retry` の戻り文言 (§3.5 の文言表の**全 6 行**。**outcome は 7 種**で、`already_decided` と `invalidated` が 1 行に併記されるため行数は 6 — r4 Minor 1 の訂正) をそれぞれの到達状態で観測する。特に **`foreign_waiting` のときに「配備まで完了しました」と言わない**こと、`foreign_waiting` と `still_pending` が**別の文言**になること、`already_decided` / `invalidated` が **`outcome.status` を写した値**を出すこと (lock 外で読み直した値ではない)。<br>**存在しない approval id に対する `approval retry`** は outcome ではなく **`ValueError("approval N not found")` の送出**で、戻り文字列は **`エラー: approval N not found` + 改行 + `_HELP` 全文** (先行する `except (ValueError, KeyError)` に捕まる — r4 I3。逐語で assert する) |
| **AC-14d** | **outcome の網羅性 (r4 I2)**: (1) **AST 検査** — `approve_candidate` と `retry_approval` の関数本体に **bare `return` / `return None` が 1 つも無い**こと (`ast.parse` で `Return` ノードを走査し、`value is None` または `Constant(None)` を落とす)。<br>**但し書き (v1.5、実装プランの着手前検証で実測)**: 走査は**入れ子関数の本体には入らない**。`approve_candidate` は内部 helper `_close_own_unfinished_journal_if_any` を持ち、**入れ子関数は `approve_candidate` 自身の `return` 契約の対象外**なので、その本体の `return` は拾ってはいけない。素朴な `ast.walk` はこれを拾って偽 red になるため、**自分の本体だけを辿る走査** (テスト側の `_walk_own_body`) を使う。<br>**v1.7 訂正**: v1.5 は除外の理由を「helper は `-> None` の手続きだから」と書いていたが、**v1.7 で helper の戻り値は `-> int | None` (閉じた `op_id`) になる** (§3.2「1 本化」) ので、この理由は使えない。正しい理由は上記の「入れ子関数だから」であり、**`_walk_own_body` の実装は変えなくてよい**。(2) **各 `return` 地点を実際に通すテスト**が、戻り値が outcome 型であり `outcome` が**既知 enum 値**であることを assert する (§3.5 の `return` 地点表の全行に 1 テスト以上を対応させる)。(3) シェルは未知 / `None` の outcome を**文言にしない** — 与えると例外になり `エラー: ...` が返る (fail loud、接頭辞が付かないことも assert) |
...
| **AC-14c** | `approve_candidate` / `retry_approval` が outcome を返すようになっても、**戻り値を使っていない既存の呼び出し元 = 本体コード 2 箇所** (reconcile → `retry_approval` / `retry_approval` → `approve_candidate`) **+ テスト 30 箇所あまり**が**書き換えなしで緑**。**v1.6 訂正**: v1.5 は本体「4 箇所」と書いていたが、retry ハンドラ (v1.2 から) と approve ハンドラ ((A) から) が戻り値を**使う**側に移ったので本体は 2 箇所 |
| **AC-15** | 既存の `tests/plugin/test_switch_journal.py` / `test_switch_paths.py` / `test_materialize_retire.py` / `tests/fixtures/test_wiring_envs.py` / `tests/test_service_app.py` が**1 本も書き換えずに緑**。書き換えるのは §7.1 の **4 本だけ** (`test_indicator_initial_set.py` の 2 本 + `test_commands.py:541` の spy + `test_reconcile.py::test_reconcile_resolution_holds_the_dependency_locks`) で、**同ファイルの他のテストは不変** (`test_reconcile.py` は v1.5 で「無改変」の側から外した — 下記 §7.1)。<br>**v1.8 の注**: ここでいう「書き換え」は**既存の期待値を変えること**を指す。**T14 は `tests/plugin/test_switch_paths.py` の 2 本に pin を 1 行ずつ「追加」するだけ**で、既存の assert を 1 つも消さず変えないので、**4 本には数えない**。対象は `test_approve_candidate_snapshot_invalid_stays_pending_with_reason` (`:433`) と `test_approve_candidate_hash_mismatch_stays_pending_with_reason` (`:461`) |
...
| **AC-17a** | **`approve <id>` (kind=plugin) が lock 内 outcome を文言に写すこと (v1.6、§3.6)**。`approve_candidate` を差し替えた spy が 7 種の outcome を返すとき、戻り文字列が §3.6.3 の表の逐語になる。**`deployed` / `deployed_after_rollback` だけが `approval #<id> approved: ` の接頭辞**を持ち、他は `approval #<id> は今回の操作では承認されませんでした (status=<status>): ` の接頭辞を持つ |
| **AC-17b** | **activity の書込は `deployed*` のときだけ** (v1.6)。`still_pending` / `foreign_waiting` / `legacy_plain_present` / `already_decided` / `invalidated` のいずれでも `APPROVAL approved` の行が増えない。**`already_decided` かつ `status == "approved"` (二重 approve) でも増えない** (§3.6.5 の 1) |
| **AC-17c** | **後続決定との競合 (AC-14b の approve 版、v1.6 の主 killer)**。`approve_candidate` が `still_pending(candidate_missing)` を返した**直後・シェルが文言を組み立てる前**に、**別コネクションの競合者が同じ approval を `approved` にする** (`approvals.apply_decision(conn2, id, "approved", ...)`。これは期限切れ sweep / 別プロセスの CLI / もう一人の運用者の決定を代表させたもの)。**観測**: シェルの報告は **`approval #<id> は今回の操作では承認されませんでした (status=pending): approved になりませんでした (reason=candidate_missing)` の逐語のまま**で、activity にも `approved` が増えない。**変異**: lock 解放後に `SELECT status, reason` で読み直す実装 (= v1.5 までの approve ハンドラ) に戻すと red |
...
| **AC-16c-4** | **監査の観測** (v1.7。**v1.8b で実フィールドに訂正**)。AC-16c-1 の実行で activity に **`switch_resume_precondition_changed`** が **1 本** (`name=` / `op_id=` / `phase=` / **`stored=(…) now=(…)`** / `stored_switch_required=` / `recomputed=` を含む — `switch.py:1731-1738`) 残り、その**直後**に `_revert_one` の `switch_reverted` が並ぶ (= 2 行になる。「ちょうど 1 行」ではない)。巻き戻した行は終端 `reverted` の**まま**で、後から `decided` に上書きされない (T2 の入口ガードと合わせた二重の網) |
...
| **AC-16c-7** | **巻き戻しの commit と新 journal 行の作成の間で落ちたときの収束** (v1.8、codex r5 #4)。AC-16c-1 と同じ状態を作り、**`begin_switch_journal` を 1 回だけ失敗させて** `approval retry` を打つ: (i) その時点で停止行は **`reverted` (終端)**、**非終端行は 0 本**、approval は `pending`、live 不変。(ii) 失敗注入を外して**もう 1 回** `approval retry` を打つと、`get_open_by_name` が `None` なので**素の新規 approve 経路**を通り、journal は **計 2 本** (旧 `reverted` + 新 `decided`)、approval は `approved`。**このときの outcome は `deployed`** (`rolled_back_op_id is None` — 巻き戻したのは**前回の呼び出し**なので、今回の呼び出しは巻き戻しをしていない)。§3.2 のクラッシュ点表「`R` の後・`S3` の前」行を実際に通す |
| **AC-16c-8** | **`old_target` の drift を検出すること (v1.8、R13 の主契約)**。**保存値も再計算値も `switch_required=1` のまま**で、`old_kind='symlink'` の行の live を第三者が **`old_target` でも `new_target` でもない第 3 の先 `Y`** へ張り替えた状態から `approval retry` を打つと: 巻き戻しが起き、journal は **2 本** (旧 `reverted` + 新 `decided`)、**新しい行の `old_target == Y`**、outcome は `deployed_after_rollback`、live は `new_target`。**v1.7 の述語 (`switch_required` の一致だけ) ではこの drift を検出できず、行が再利用されて `old_target` が古いまま残る** — 後の巻き戻しが第三者の `Y` ではなく古い値を復元してしまう (R13 の存在理由そのもの) |
| **AC-19a** | **版 dir の照合が `switch_required=0` でも走ること (v1.8、R14 / IV-7 の主契約)**。`switch_required=0` で `recorded` まで進む状態を作り (live が既に候補の `new_target` を指している同一候補の再 approve)、**版 dir の中身を第三者が in-place 編集**してから `approve` / `approval retry` を打つと: approval は **`pending` のまま**、戻り値は **`still_pending(reason="reverify_failed")`**、**自分の未完行は閉じられて非終端 0 本** (名前が凍らない)、**live は 1 バイトも変わらない**。**現行 (v1.7 まで) は照合を通らず `approved` になる** |
...
| **AC-19d** | **bless への波及 (v1.8、§3.7.3)**。`switch_required=0` の bless で版 dir が壊れていると **`RuntimeError`** が上がる。**`bless_candidate` を直接呼んだ場合は `RuntimeError` がそのまま上がる**ことを観測する。**v1.8b 訂正**: 「CLI は rc=1 + `エラー: ` (traceback ではない)」は現物と合わない — `_plugin_bless` (`cli.py:600`) の `except` は `(ValueError, plugin_sandbox.SandboxError)` のみで `RuntimeError` を捕らない (外側の包括 catch `cli.py:781-784` にも `RuntimeError` は無い) ため、**CLI 経由では traceback が出る** (`switch_required=1` の同じ `RuntimeError` は本束以前から同じ挙動 — 新しい後退ではない)。CLI の表示を rc=1 化する是正は本束の範囲外 (Global Constraints が本束で足す `except` を 3 つに限定しており、T13 の担当は `switch.py` のみ) — 別途 ticket **[cli-bless-runtime-error-traceback]** で追う。**このとき bless は自分の未完行を閉じない** (設計どおりの残余) ことを観測し、**解除手順として `reject_candidate` で成立する**ことまでを 1 本で通す (`reject_candidate` は `existing_journal["approval_id"] == approval_id` の判定 (`kind` を見ない、`switch.py:1921`) で bless の残余行も自分の未完行として拾い `_revert_one` で閉じる) |
| **AC-19e** | **(v1.8a 新規、codex sol r6 #1) bless の残余が `approval retry` でも解除できること**。AC-19d と同じ状態 (bless の `recorded` 行が非終端で残る) を作ったあと、`reject` ではなく **`approval retry <approval_id>` を打つ**と: `retry_approval` は `approve_candidate` を透過呼出しし (`:1816-1829`)、0d の `existing_journal["approval_id"] == approval_id` 判定 (`:1567`) は **`kind` フィールドを見ない**ため bless が作った行も approve の 0d が拾う。**v1.8b 訂正**: 「版 dir の中身が一致していれば `decided`/`approved`、一致しなければ `still_pending(reverify_failed)`」は**一致しない側が現物と合わない** — `_advance_to_decided` は `record_version` (§3.7.1) を毎回無条件で呼び直すため、**版 dir を直さずに retry すると、T13 の switch_required=0 分岐に到達する前に `record_version` 自身が改竄済みの版 dir を独立に再照合して `history_git.HistoryGitError("... index blob hash mismatch ...")` を送出する**。この例外は `switch.py`・`retry_approval`・呼び出し元のどこにも catch されず上へ抜け、**残余行は非終端のまま残る** (`still_pending` への収束はしない。plugin 名は凍ったまま)。解ける条件は**版 dir を正しい中身に戻してから `approval retry` を打つ**、または `reject <id>` の 2 つ (`still_pending` 経由の自動解除ではない)。**版 dir を直した側は従来どおり `decided`/`approved` まで完遂し、非終端行は残らない** — こちらは訂正なし |
...
| `tests/test_commands.py:541-565` (`test_approval_retry_dispatches_to_switch_retry_approval`) | `retry_approval` を **`None` を返す spy** に差し替え、`calls == [(42, "shell")]` / `"再試行" in out` / activity `retry` | **書き換え** (v1.4、r4 I2)。**spy が実物より緩い** (実物は必ず outcome を返す) ため、`None` fallback を置かない方針では `AttributeError` になる。spy を**実物と同じ形の outcome を返す fake** に直す ([[test-fixtures-from-real-transcripts]])。`calls` と activity のアサーションは不変、`"再試行" in out` も新文言の接頭辞で成立する |
| `tests/plugin/test_reconcile.py::test_reconcile_resolution_holds_the_dependency_locks` (`:473`) | `acquired == sorted({"rsi_pullback", "rsi"})` — **取得した lock 名の列を完全一致**で pin | **書き換え** (v1.5、実装プランの着手前検証で実測)。§3.3.3 の 3 箇所目 (pin 破れの巻き戻し) が `_plugin_lock` を 1 本増やすため、末尾に巻き戻し用の `"rsi_pullback"` が付く。この pin の主旨 (**解決が依存 lock の内側であること**) は不変で、期待値を `[*sorted({"rsi_pullback", "rsi"}), "rsi_pullback"]` に更新するだけ。**同ファイルの他のテストは不変** |

**`None` を返す spy の全数確認 (v1.4)**: `retry_approval` / `approve_candidate` を差し替えているテストは
`grep -rn 'setattr(.*\(retry_approval\|approve_candidate\)'` で 2 件のみ —
`tests/test_commands.py:558` (上記、書き換え対象) と `tests/plugin/test_switch_journal.py:214` (`_boom`、
**例外を送出する fake** なので戻り値契約に依存しない)。
`tests/plugin/test_switch_journal.py:83` の lambda は `None` を返すが、**呼び出し元は reconcile** であり
reconcile は戻り値を無視する契約 (§3.5) なので**不変**。
...
| `tests/test_commands.py:127-157` (`test_approve_plugin_kind_reports_actual_outcome_not_always_approved`) | `out != f"approval #{id} approved"` / `"pending" in out` | **v1.6 で `approve` ハンドラを変更するが、このテストは無改変で緑のまま。** 根拠: 候補欠損は `still_pending`(`status="pending"`) を返すので、新文言は `approval #<id> は今回の操作では承認されませんでした (status=pending): approved になりませんでした (reason=candidate_missing)` になり、**`"pending" in out` は `status=pending` で成立**、`out != "approval #<id> approved"` も成立する。**接頭辞に `(status=...)` を置いたのはこの互換のためでもある** (§3.6.3) |
| `tests/test_commands.py:113-125` (`test_approve`) | `"approved" in out` / DB が `approved` / activity | **非 plugin kind** (`kind="tech_plugin"`) なので `approve_candidate` を通らない。§3.6.4 の 1 行目 |
| `tests/test_commands.py:187-201` (`test_approve_nonexistent`) | `"存在しません" in out` | `SELECT kind` が `None` の行は plugin 枝に入らないので、`approve_candidate` の `ValueError("approval N not found")` には到達しない。§3.6.4 の 2 行目 |
...
| `tests/plugin/test_switch_paths.py:433` / `:461` (`snapshot_invalid` / `hash_mismatch` の留置) | `row["status"]` (DB) と `outcome.outcome` / `outcome.reason` | **期待値は不変**。**T14 (v1.8) が `assert outcome.status == "pending"` を 1 行「追加」するだけ** (2 周目ローカル LLM T2-01 — `status` を `"approved"` に潰す変異が SURVIVED する) |
...
| ~~`tests/plugin/test_reconcile.py:443`~~ | ~~`_unresolved_after_switch` が依存 lock を握ること~~ | **v1.5 で「不変」から「書き換え」へ移した** (上表)。主旨 (解決が依存 lock の内側) は不変だが、`acquired` の**完全一致**の期待値が 1 本増える |
...
| **「plugin 名が凍ったとき」(v1.7 で新設)** | 記述なし | **1 節を足す**: `bless` / `retire` / 別 approval の `approve` が `UnresolvedJournalError` で拒否されたら、**`approval list` → `approval retry <approval_id>`**。retry は (i) 切替が途中で止まった行、(ii) **再開の前提 (`switch_required`) が resume 時点で崩れた行** (§3.2、第三者が live を触った場合) のどちらも 1 回で収束させる。**起動時 reconcile では解けない** — 非 `switched` 行は飛ばす仕様なので、サービスを再起動しても凍結は解けない。**`foreign` (live が第三者に触られた `switched` 行) のときだけ人間が判断する** |
| **「版 dir の中身が候補と合わないとき」(v1.8 で新設)** | 記述なし | **1 節を足す**: `approve` / `approval retry` が `approved になりませんでした (reason=reverify_failed)` を返す、または `bless` が `エラー: ... content_hash mismatch ...` で rc=1 になったら、**`plugins/.versions/<name>/<artifact_hash>/` の中身が候補と食い違っている** (第三者による in-place 編集の疑い)。**機械は自動で作り直さない** (§3.7 の裁定)。人間が (i) 版 dir の内容を確認し、(ii) 壊れていれば**その版 dir を消して** `approval retry` を打つ (`create_version_dir` が候補から作り直す)。bless 経路では未終端行が残るので **`approval list` → `approval retry` / `reject`** で先に収束させる |
...
なし (§0 の R1〜R8 と、**§0 の R9 / R10 / R11 / R12 (2026-09-20 追加裁定)** で確定済み)。

**v1.7 の申告 1 点 (`[resume-stale-old-target-not-detected]`) は v1.8 で解消した** —
codex sol r5 #2 が同じ論点に「強い述語を採るべき、新たに壊れる正常系は見当たらない」と回答し、
**ユーザー裁定 R13 で 3 つ組の述語を採用した** (§3.2)。ticket は起票しない。
**v1.7 の残余 1 (`[switch-not-required-skips-hash-verify]`) も R14 で閉じる** (§3.7)。

**v1.7 で既存テストの書き換えは増えない (確認した)**: T11 は「前提が崩れていない再開」の挙動を
1 バイトも変えない (AC-16c-3 が回帰防止として pin する) ので
`tests/plugin/test_switch_paths.py:677 / :1119 / :1160` (`recorded` からの retry) は不変。
T12 は **CLI の `_plugin_retire` の失敗経路を踏む既存テストが 1 本も無い**
(`grep -rn "_plugin_retire\|\"retire\"" tests/` = 0 件) ため不変。**v1.8 でも増えない (確認した)**: R13 は「前提が崩れていない再開」を変えない。R14 は `switch_required=1` の枝を 1 行も変えないので既存 pin `tests/plugin/test_switch_paths.py:569` / `:1401` (`RuntimeError`) はそのまま、`switch_required=0` の正常系 (`test_runbook_post_gate_failure_converges_via_approval_retry` 手順 5 / AC-16b) は版 dir が候補と一致するので照合を通る。`_advance_to_decided` の戻り値変更は、直接呼ぶ唯一の既存テスト `tests/plugin/test_switch_ops_hardening.py:513` が戻り値を使っていないので無影響。**§7.1 の書き換えは 4 本のまま。** **T14 (v1.8) が `tests/plugin/test_switch_paths.py` の 2 本に pin を「追加」するのは、既存の期待値を 1 つも変えないので書き換えに数えない** (AC-15 の注)。
...
### 8.4 設計レビュー r4 の処置 (codex terra、`tmp/design-switch-ops/codex-design-r4.md`、C0 / I3 / M1)

**4 件とも採用**。いずれも契約の記述の精度に関する指摘で、設計の骨格 (分類器 / 案 C / 2 段ガード / lock 内 outcome) は動いていない。

| # | 指摘 | 事実確認の結果 | 処置 |
|---|---|---|---|
| I1 | `invalidated` の `status` は「lock 内で読んだ値」ではなくリテラルで、出所契約と記述が一致しない | **そのとおり** (`switch.py:1397-1417` — lock 内の再読は非 pending 判定の `:1398-1401` だけ) | `status` の定義を「**lock の内側で確定した値** = (a) read か (b) 同じ lock 内で成功した `apply_decision` の結果」に精密化。`return` 地点表の**全行に出所 (read / decision 結果) を明記**。「lock 外で読み直さない」は不変 |
| I2 | `None` / 未知 outcome の fallback が、本体の `return` 漏れを普通の応答に化けさせる | **そのとおり**。本束が直そうとしている「表示が実態と食い違う」を新しい場所で再生産する | **方針変更 — fallback を撤回**。シェルは既知 enum 以外を文言にせず fail loud。`tests/test_commands.py:541` の spy は「実物より緩い fake」なので**実物と同じ outcome を返すよう書き換える** (§7.1、**書き換え 2 本 → 3 本**。→ **v1.5 で 4 本**)。**AC-14d を新設** (AST で bare `return` / `return None` が無いことの検査 + 全 `return` 地点を通すテストが outcome 型と enum 値を assert + 未知/`None` が文言にならないこと)。`None` を返す spy の全数確認も §7.1 に記録 |
| I3 | approval 不在の `ValueError` の捕捉先が実コードと違う (包括 `except Exception` ではなく先行する `except (ValueError, KeyError)`、表示に `_HELP` が付く) | **そのとおり** (`commands.py:292-295` — `except (ValueError, KeyError)` は `f"エラー: {...}\n{_HELP}"` を返す) | §3.5 の記述を訂正し、**例外 → `except` → 戻り文字列の対応表**を新設 (`ValueError` / `UnresolvedJournalError` / `RuntimeError` / `OSError` の 5 行)。AC-14a を**実際の戻り文字列** (`エラー: approval N not found` + 改行 + `_HELP`) に合わせた。**retry だけ別扱いの 1 行契約にはしない** (現行の表示を変えない) |
| M1 | AC-14a の「outcome 6 種」が列挙 (7 種) と不一致 | **そのとおり** | 「**outcome 7 種 / 文言カテゴリ 6 行**」(`already_decided` と `invalidated` は 1 行に併記) と明記。fallback 行は I2 で削除 |

### 8.5 2 周目 `/code-review high` の処置 (v1.7、`tmp/review-20260920-soh/r2/code-review.md`)

`main..switch-ops-hardening` 全体に対する 10 件のうち、**本束で是正するのは #1 と #5 の 2 件**
(2026-09-20 ユーザー裁定)。#1 は**指揮者側の probe で再現を実測**した
(`tmp/review-20260920-soh/r2/cr1-probe.md`、`tmp/review-20260920-soh/r2/cr1-probe/test_probe_cr1.py` = `4 passed`)。

| # | 指摘 | 事実確認の結果 | 処置 |
|---|---|---|---|
| #1 | `_finalize_decision` の phase ガードが行の**作成時**の `switch_required` を読むため、非 `switched` phase で止まった行を再開すると、その回に再計算された値と食い違って approve / retry が永久に詰まる | **再現した (Important)**。停止 phase `preparing` / `versioned` / `recorded` の 3 通りとも詰まり、reconcile・expire・再起動のいずれでも解けない。`reject` だけが抜け道で、しかも live は第三者の状態のまま残る。**逆向き (保存 0 / 再計算 1) は `advance_switch_journal` の既存ガードで詰まる別経路**で、これは本束以前からの欠陥。なお**本束以前の挙動は「正しく finalize できていた」のではなく fail open** (第三者が置いた symlink の中身を無検証で approved にしていた) — **ガードを戻す方向の是正は採らない** | **案 3 を採用** (§0 R11)。§3.2 に「#### 再開の前提が崩れていたら…」を新設 — 0d の**全分岐表**で判定を置く 2 箇所 (plain 分岐の手前 / `switch_required` 再計算の直後) を特定し、`_close_own_unfinished_journal_if_any` に 1 本化、outcome は既存の `deployed_after_rollback` に合流、activity に専用イベント。§3.2 のクラッシュ点表に T11-B の段を追加。§3.5 の `rolled_back_op_id` の契機を 2 つに。§4 の全経路表に「呼び出し元は 5 箇所のまま」を追記。§5 に残余 2 件。**AC-16c-1〜6 を新設** |
| #5 | bless に足した「次の一手」が `_plugin_retire` の同型ハンドラ (`cli.py:628`) に無い — 同じ原因に違う案内 | **そのとおり**。`retire_plugin` の例外文言は `op_id=` のみで **`approval_id=` を含まない** (`switch.py:1795-1798`) ため、案内の第一手が `approval list` であることに意味がある | §3.4 に retire の行と文言を追加 (**既存の 1 節を 2 節に割るだけで捕捉集合は不変**)。**AC-18 を新設** (§0 R12) |
| #2 (`reject_candidate` が `classify_live` を通らない) | 既知 | 既に ticket `[reject-revert-without-live-classification]` (IV-6 の例外 2、v1.6a)。**本束では直さない** |
| #3 / #4 / #6 / #7 / #8 / #9 / #10 | compare-and-set の共通化 / `except ValueError` の広さ / outcome 構築のコピペ / `status` の Literal 化 / `approvals.pending` の再利用 / `payload_json` パースの重複 / 冗長な DB 読み | いずれも altitude・reuse 系で**正しさの risk は無い** (レビュアー自身の要約もそう述べている) | **本束では扱わない** (2026-09-20 ユーザー裁定 = 採用は #1 と #5 のみ) |

### 8.6 設計レビュー r5 の処置 (v1.8、codex sol、`tmp/review-20260920-soh/r2/design/codex-out.md`、C0 / I5)

v1.7 の T11 / T12 に限定した設計レビュー。一次トリアージは
`tmp/review-20260920-soh/r2/design/codex-triage.md` (**確定 4 / 誤検知 1**)。

| # | 指摘 | 事実確認の結果 | 処置 |
|---|---|---|---|
| #1 | activity 記録が失敗すると食い違った journal を閉じられない | **誤検知**。`src/agentic_fx/activity.py:32-65` の `ActivityLog.write` は **「例外を送出しない」契約が明文化**されており (docstring に N1 / N2 の是正履歴)、整形処理も含めて `try/except Exception` で全て捕捉し、失敗時は技術ログへ warning するだけ。呼び出し元 (`_revert_one` / T11 の判定) が依存する「`activity.write` は投げない」は**既存の不変条件として現物に存在する**。codex は材料に `activity.py` を含めていなかったため見落とした | **対応不要**。本表に判定と根拠を記録する (再提起されたときの参照点) |
| #2 | 判定述語を `(switch_required, old_kind, old_target)` に強めるべき | **確定 (技術的根拠あり)**。`old_target` は監査情報ではなく `_revert_one` の補償データ。v1.7 は同じ drift を「既知の残余 2」として認識した上で R11 の範囲外としていたが、codex は**指揮者が投げた問い 4 への回答**として「強い述語を採るべき、新たに壊れる正常系は見当たらない」と判定 | **採用 (ユーザー裁定 R13)**。§3.2 の述語を 3 つ組に。**副産物として判定が 1 箇所に畳めた** (plain は `old_kind` の不一致として自動的に拾われるので、v1.7 の T11-A 専用分岐が不要になった)。`old_target` の保存形式を現物で確認し**正規化不要**を明記。AC-16c-1〜7。ticket は起票しない |
| #3 | T11 と同時に hash 再照合を入れないと fail-open を再導入する | **確定**。v1.7 の「既存の穴に戻るだけ」という評価は**変種 (a) に限っては不正確** — **現行 HEAD (T11 前) は変種 (a) を `ValueError` で遮断しており到達不能**、T11 はそれを**到達可能な `approved` 経路に変える**。比較対象を「本束以前」ではなく「現行 HEAD」に取るのが正しい | **採用 (ユーザー裁定 R14)**。**§3.7 を新設** — 照合を `if switch_required:` の外へ出して必ず通す / 処置は収束可能性で分岐 / **修復はしない (裁定の文言どおり pending 留置)** / outcome は既存 `reverify_failed` に合流 / bless への波及と残余。**IV-7 を新設**。AC-19a〜d。ticket は起票しない |
| #4 | 巻き戻しの commit と新 journal 行の作成の間のクラッシュ収束が AC で固定されていない | **確定 (テストカバレッジの穴)**。§3.2 のクラッシュ点表は「`R` の後・`S3` の前」行でナラティブに説明しており主張自体は正しい (`get_open_by_name` が `None` → 素の新規経路) が、**それを実際に通す AC が無かった** | **採用**。**AC-16c-7 を新設** (`begin_switch_journal` を 1 回だけ失敗させ、2 回目の retry で計 2 行・`approved`・**今回の outcome は `deployed`** まで観測する) |
| #5 | AC の phase 行列が逆向き (c) と plain で `recorded` 固定になっている | **確定**。AC-16c-1 は 3 phase を明示しているのに AC-16c-2 / AC-16c-6 は `recorded` の 1 通りだけで、**実装が `phase == "recorded"` のときだけ判定する退化を殺せない** | **採用**。**AC-16c-2 / AC-16c-3 / AC-16c-6 を 3 phase parametrize に広げた** (AC-16c-3 は回帰防止なので同じく 3 通りで押さえる) |

## 9. 変更履歴

| 日付 | 版 | 変更 | 理由 | commit |
|---|---|---|---|---|
| 2026-09-20 | v1.8c | **2 周目やり直し `/code-review high` r2b (`tmp/review-20260920-soh/r2b/code-review.md`、5 件) の処置を記録**: **#1・#2 = 既知**、ticket **[cli-bless-runtime-error-traceback]** の範囲 (本束では対処しない) / **#5 = R14 の意図どおりの設計** (誤検知ではなく仕様確認) / **#3・#4 を採用**し `b126dd2` で是正済み — **#3**: `bless_candidate` の `switch_required=0` 枝 (`_advance_to_decided` が `False` を返したとき) の `RuntimeError` 文言が `after switch` (切替後) と言っていたのは事実と不一致 (`switch_live` を一度も呼んでいない) だったので「版 dir の content_hash mismatch」に訂正 (`content_hash mismatch` 部分文字列の既存 pin (AC-19d) は維持、`switch_required=1` 側の文言・pin は無改変) / **#4**: `cli.py` の `_plugin_bless` / `_plugin_retire` に重複していた収束手順案内2行を `_UNRESOLVED_JOURNAL_RECOVERY_HINT` 定数へ切り出し。**加えて codex sol r2b の設計レビューは C0/I0/M0** (確定なし)。§3.7.3 に文言の決めを追記 (この行) / 本是正の pin (`tests/plugin/test_switch_ops_hardening.py::test_ac19d_*` への assert 追加) と `_UNRESOLVED_JOURNAL_RECOVERY_HINT` 直上コメントの `T6`→`T6 / T12` 訂正は本 commit と同時に別 commit で追加 | ユーザー裁定 (**#3・#4 は維持**) を受けた記録是正。是正コード自体は `b126dd2` (別セッションによる先行実装、TDD・変異確認なしのため本束で pin と手戻し確認を補完) | `b126dd2` |
| 2026-09-20 | v1.8b | **T11 (`4df5207`) / T13 (`cd792f5`) 実装時の実測で判明した設計記述の誤りを訂正** (docs のみ、実装は変更なし): **(1)** §3.7.1 / §0 R14 — `history_git.record_version` は switch_required に関係なく毎回、版 dir の実ファイルを候補の `(content_hash, artifact_hash)` と独立に照合しており (`history_git.py:135-138` / `:161-172`)、「`switch_required=0` は無検証で approved」は過大。**R14 (`_version_dir_hashes_ok`) が実際に塞ぐのは `record_version` の照合後〜`_finalize_decision` 前の TOCTOU の窓**だけ (多層防御として実装は残す) / **(2)** AC-19d — CLI は `_plugin_bless` の except が `RuntimeError` を捕らないため **traceback が出る** (rc=1 ではない)。訂正した観測は `bless_candidate` への直接呼び出しに絞り、CLI の rc=1 化は ticket **[cli-bless-runtime-error-traceback]** へ切り出し (本束の範囲外) / **(3)** AC-19e — 版 dir を直さずに `approval retry` すると `still_pending(reverify_failed)` にはならず、`_advance_to_decided` の switch_required=0 分岐に到達する前に `record_version` 自身が `HistoryGitError` を送出して残余行は非終端のまま残る。解除は「版 dir を直してから retry」または `reject` の 2 つ / **(4)** T11 変異表 (実装プラン T11-M11/M12) の killer を実測に合わせ訂正: M11 (3 つ組→`old_kind` のみ) の killer は `test_ac16c8_*` のみ、M12 (→`old_target` のみ) の killer は `test_ac16c6_*` のみ (プランが書いていた `test_ac16c1_*`/`test_ac16c2_*` は実測では red にならなかった)。M6 (`_close_own_unfinished_journal_if_any` の `conn.commit()` 削除) は T11 の再開判定経路では直後の `begin_switch_journal(commit=True)` に吸収されて等価、M9 (`bool()` を落とす) は sqlite の int 0/1 と Python の bool 比較が等価なため等価 — いずれも実測 (フルスイート + `-k ac16c`/`ac9d`) で確認 / **(5)** activity イベント `switch_resume_precondition_changed` の実フィールドを `stored=(…) now=(…) stored_switch_required=… recomputed=…` (`switch.py:1731-1738`) に訂正 (§3.2 本文・AC-16c-4)。**設計判断・実装は 1 つも変えない** (docs 訂正のみ) | 実装プラン T11/T13 の担当による着手前検証・実測報告 (worktree `tmp/wt/soh` HEAD `cd792f5` で全項目を現物照合)。M11/M12/M6/M9 は `tests/plugin/test_switch_ops_hardening.py -k ac16c` / `-k ac9d` を実際に走らせて再確認 | — |
...
| 2026-09-19 | v1.4 | 設計レビュー r4 の 4 件を採用: `status` の出所を「lock 内で**確定**した値 (read / decision 結果)」に精密化し `return` 地点表の全行に明記 (I1) / **`None`・未知 outcome の fallback を撤回**し fail loud + **AC-14d (網羅性)** を新設、`tests/test_commands.py:541` の spy を書き換え対象に (書き換え 2 本 → **3 本**) (I2) / 例外 → `except` → 戻り文字列の対応表を新設し AC-14a を実文字列に (I3) / outcome 7 種・文言 6 行の訂正 (M1)。AC は 25 → 26 項目 | `tmp/design-switch-ops/codex-design-r4.md` (C0 / I3 / M1)。`commands.py:292-295` と `switch.py:1397-1417` を実コードで確認 | — |
