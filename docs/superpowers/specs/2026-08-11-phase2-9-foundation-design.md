...
- **【設計欠陥】外向きリクエストの予算という制約クラスが存在しない (2026-08-12 起票・要設計)**

  **これは実装の抜けではなく設計の欠陥。** 本設計書を含む全設計文書で「レート制限」は
  ①シグナル起動の内向き制限 ②Claude SDK から制限される側の扱い、の 2 つしか扱っておらず、
  **自分が第三者サーバへ投げる量を律する記述が一つも無い**。一方で
  `2026-07-25-agentic-fx-design.md:494` は Dukascopy を「長期 1m の一次ソース
  (無料公開、10 年超)」と位置づけている — **設計が意図した本線の使い方が、
  そのまま相手方の遮断を招く構造**だった。

  **実害が顕在化済み**: 開発機は Dukascopy に遮断された (別 IP では同一 URL が通ることを
  対照実験で確認)。10 年分 = 約 87,600 リクエスト。**リポジトリは公開済みで、
  クローンした人が README どおりに動かせば同じ目に遭う。**

  **応急封鎖は `4a08226` で投入済み** (Dukascopy のみ: 1 秒間隔・500 本上限・429/503 で即中止)。
  **本設計は未着手。** 対象は Dukascopy に限らない — 第三者サーバへ出る経路は
  `datafeed/sources.py` (twelvedata)、`datafeed/fetchers.py` (news 任意サイト)、
  `datafeed/econ_calendar.py`、`core/notifier.py` (Discord) がある。
  **今それらが無事なのは設計のおかげではなく、たまたま呼び出し頻度が cron 周期や
  承認済みソース数に律されているだけ** — そこにループを 1 つ書けば同じ穴が開く。

  設計時に決めること: ①制約を Global Constraints 級に格上げするか (kill switch と同様に
  config で無効化不可にするか) ②第三者向け出口を共通ラッパへ集約する範囲 (localhost の
  MT5 bridge / llama-swap は対象外でよいか) ③既定値を安全側に置く方法 (未設定なら
  保守的な既定で動き、設定し忘れが連射にならない) ④素性を名乗る User-Agent
  ⑤429/503 のバックオフ方針 (応急封鎖は即中止にしている)。
