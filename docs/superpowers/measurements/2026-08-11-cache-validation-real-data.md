...
- **測定範囲**: 本計測が再現するのは `_cached_bars` の**直読枝** (`price_provider.py:230` の `src == interval` — floor しない `window_start` を下限に使う) のみ。導出枝の `derive_since` (要求 interval へ floor した下限) は対象外 — script は resample を行わないため構造上その枝に到達しない。また `now` を「最終バー + 1 足」で作るため鮮度検査 (`freshness_max_min + interval_min` = 21 分に対し `now - last_ts` = 1 分) は前後とも構造的に必ず PASS する — 下表の「合否」欄が動きうるのは gap/zero/spike 検査のみ
