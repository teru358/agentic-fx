...
| 2026-09-13 | v1.11 | A4 17 で確定した未実装 2 件を是正 (branch profitability-floor-fix → main ff): F8-4 フロア不合格 activity に閾値 3 値 (`floor_settings_kv` 共有) / F4-4 `StrategyGateVerdict.holdout_metrics` + `_render_floor_report_detail` (8 指標 × 段 × pair + 閾値)。codex 差分レビュー: 実装欠陥 0、pin 強化 2 (activity 負の pin、2 pair 完全性) 採用。fresh フルスイート 3873 passed (既知 shell_interrupt 9)。approval #11 は reject 済 (`tmp/approval-20260913.md`、T0 実機 OK、.locks sweep 実機 OK) | `tmp/review-20260912-pf/{impl-report-fix.md,mut-pf-fix.json}` | 0caa2ad |
| 2026-09-13 | v1.12 | 実機 A4 18/19 回目 (codex、`tmp/a4-run18-codex-20260913.md` / `tmp/a4-run19-codex-20260913.md`): **A 経路 (holdout 段フロア不合格) を誘導で 2 例取得 (#83 / #84)**。run18 = `min_pf 1.3` のみ → agent が不採算系統 #8 を選び `submission_blocked` 3/3 で observation 終端 (P1a 充足、A 経路は条件未発生)。run19 = `min_pf 1.3` + `improve add` 1 回 (backlog #69、SMA 10/30 を SL/TP 2 通りで必ず提出) → in_sample pf 1.4965 合格 → holdout pf 0.8075 で `gate_failed reason=unprofitable min_pf=1.3 require_positive_avg_r=True require_holdout_evaluable=False` (**F8-4 実機実証**) / `last_result='unprofitable'` 12 B 完全一致 / gate 行 in_sample 4 + holdout_gate 1 が全て `mission_outcome='unprofitable'` / approval 11 件不変 / report `data/improve_reports/improve-2026-09-13-83.md` に floor detail 節 (failed_stage/failed_pairs/閾値 + in_sample・holdout × pair 8 指標、**F4-4 実機実証**) / INDEX status `unprofitable` / staging 削除。2 回目 mission #84 の prompt で履歴表の #69 行が `unprofitable` の 1 bit のみ (遮断 8 本番動作) かつ `holdout` 0 件。閾値 `1.0` の生存なし、新規欠陥 0。**P3a〜P3h 全充足、P1a/P1b/P2/P4/P5/P6 も run17〜19 で全充足 → 実機観測完結**。方法論の観測: holdout 0.8075 < 1.0 なので誘導の実効レバーは `improve add` 単独 (1.0 ≤ pf < 1.3 の識別サンプルは未取得)。申し送り: 観測 D = フロア経路では成績一致降格 (`duplicate_metrics`) が走らず #84 が #83 と指標ビット一致の候補を再提出 (→ tickets [floor-path-skips-duplicate-metrics])、観測 C = INDEX の `best= pf=` は in_sample 値で `unprofitable` 行と並ぶと誤読しうる | 実機記録 | - |
