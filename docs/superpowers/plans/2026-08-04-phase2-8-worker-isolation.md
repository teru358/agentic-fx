...
- 全体指揮 + 実装監督: opus (main セッション)。実装担当: haiku (機械的 task = Task 1-4, 9, 11, 17 目安) / codex (重量 task = Task 7, 10, 13-16, 19 目安)。レビューは下記「レビューの段構成」を正とする。変異検証: haiku 並列 fan-out
...
- **レビューの段構成 (2026-08-09 ユーザー裁定 — これが正。旧「codex 主査 + sonnet 副査」「sonnet + ローカル 2 本の 3 者」および codex 残量に基づく時限配分は全て破棄した)**:

  | 段 | 担当 | Claude 枠 |
  |---|---|---|
  | **段 0** | **指揮者の変異スイープ** (レビュー**前**に完了させる) | ほぼゼロ |
  | **1 周目** | **codex + ローカル 2 本 (並列)** | **ゼロ** |
  | **2 周目** | **`/code-review high <branch>`** + ローカル 2 本 | 約 110〜130k |
  | **3 周目** | ブリーフ付き sonnet — **2 周目で must-fix が出たときだけ**監視役に相談 | 約 55k |

  - **2 周目までを既定**とし、3 周目は条件付き。**周ごとに最適なレビュアーが変わる**のが使い分けの核 — ①1 周目は既知事項が無く**除外リストを作れない**ので sonnet の優位性が無い ②2 周目の `/code-review` はノーブリーフ前提の設計なので、既知を渡せないことが不利にならない ③3 周目は除外リストが長くなり「2 周目の修正が新しい穴を作っていないか」という**指定できる問い**が生まれる (sonnet の独壇場)
  - **実測**: sonnet 1 周目 (除外リスト無し) 120,949 トークン / 指摘 2 件、sonnet 3 周目 (除外リスト付き) 53,025 / 1 件 + 全観点の確認、`/code-review` 3 周目 (別 task) 146,367 / 6 件。**周を間違えると倍額になる**
  - **有償 2 本 (`/code-review` と sonnet) は並列にしない** — 逐次なら sonnet に渡す除外リストが効き、実測で 121k → 53k に下がる。**無償 (codex・ローカル) は並列でよい**
  - **段 0 を先に回すのが最大の効き所。** 「実装は正しいがテストが守っていない」型は機械的に取れるので、レビュアーに探させると 10 万トークン単位を使う。実測 (Task 14→16): レビュー費 488k → 274k → **109k**、指摘は 20 → 23 → 8 件
  - **段 0 スイープと外部レビューを同じ worktree で並列に走らせない** (Task 17 実測: codex が変異中のコードを見て「作業ツリーに変異が残っている」を Critical で報告し、1 周分を空振りに使った)。**変異を注入する主体は同時に 1 つだけ**
  - **1 周目・2 周目のレビューと修正は、指揮者がユーザーの許可を得ずに実施してよい** (2026-08-08 ユーザー裁定)。節目確認はマージ前に一度だけ行う
  - **3 周目の要否は監視役 (ユーザー) が判断する。** 指揮者は 2 周のレビューと修正を終えた時点で必ず停止し、マージへ進まずに判断材料を提示して指示を仰ぐ: ①2 周目で何が見つかったか (特に**修正ラウンドが持ち込んだ欠陥**の有無) ②2 周目の修正の規模と性質 (挙動変更を含むか) ③未レビューのまま残る差分 ④変異テストの生存状況
    - 根拠: Task 7 の 2 周目で、**1 周目の修正が持ち込んだ新しい欠陥** (transport 失敗後の同一 seq 再送) が実際に見つかった。「修正ラウンドを誰もレビューしない」という穴は 1 段深いところにも同じ形で存在する — 打ち切り位置を指揮者が自動で決めるのは不適切。また Task 4 の修正後、指揮者が目視で「`ValueError` の直後に到達しない分岐が残る」欠陥を発見している (`cfd0bbd`)
  - **ローカル LLM 2 本 = `qwen3-coder-30b-a3b-instruct` + `KAT-Coder-V2.5-Dev` (Q4_K_M) 常設。** コストゼロなので**指揮者の判断で黙って外してはならない** (Task 15 で無申告の省略が発覚)。材料の切り出しが難しい task でも、切り出せる範囲に絞って必ず投げる
    - **実行**: Claude Code 経由は不可 (起動時プロンプトが窓 64k を超える)。直接 API (`http://127.0.0.1:8080/v1/chat/completions`) へ**逐次**投げる (llama-swap は 1 本しかロードしない)。`reasoning_effort: "none"` 必須 + `temperature 0.6 / top_p 0.95 / presence_penalty 1.0`
    - **材料は 4〜6k トークン・全文形式。** diff 多用 (13.5k) にしたら誤検出が 4 件出た (Task 12) — diff は「変更後の姿」が読めず制御フローの誤読を誘う
    - **指摘はそのまま採用しない。** 材料の外は見えないので指摘は仮定形で止まる。**裏取りは必ず指揮者が変異で行う**
    - **KAT の `[Critical]` ラベルは無視する** (Task 15 r2 / 15 r3 / 16 r1 / 17 r2 と 4 回連続で誤り)。一方「**この行を消してもテストが通る**」形の列挙は当たるので、**列挙だけを実測対象として拾う**
    - **ローカルが構造的に取れないのは「文脈依存」だけでなく「探索が要る指摘」全般。** 「その順序を守らせているものが無い」型は sonnet / `/code-review` のみ
...
- **指揮者検証はほどほどに** (2026-08-08 ユーザー指摘)。指揮者が先に全部潰すとレビューが追認になり、**交差の価値 (別の観点から独立に見る) が失われる**。**続ける**: 現物照合・機械照合・全体テスト・プランの変異リストの裏取り・レビュアーの誤検出の反証。**控える**: 自主変異の網羅的掃討 (2〜3 件の抜き取りに留める)
...
- Produces:
  ...
  - `App.close(self, *, busy_resources: frozenset[str] = frozenset()) -> list[str]` — 停止状態機械の資源終端 (§5 手順 2〜6)。戻り値は「close をスキップした資源名のリスト」(空なら全て close 完了)。**(裁定書 FC-2 前半申し送り) `instance_lock` の close (flock 解放) もこの resources リストに含める** — Task 11 で `App.instance_lock` フィールドは追加済みだが解放は本 task に委ねられている
    ...
    - **(Task 17 レビュー 2 周からの申し送り)** **`select` が ready を返しても `read1` がブロックしない保証は無い。** `entry.py:28` により対話モードは stdin が tty のときだけ走るが、`plugin/approval.py:181` の `subprocess.Popen(...)` は **`stdin=` を指定せず fd 0 を継承**する。子プロセスが `select` と `read1` の間にバイトを奪うと **`read1` が無期限ブロックし `stop_event` が観測できなくなる** — **Task 17 の seam が消そうとした失敗モードそのもの**。`input()` 時代は「盗まれると待ちが長引く」だけだったが、いまは**中断契約を破る**。対策候補: ①読み取り前後で `os.set_blocking(fd, False)` ②`approval.py` の `Popen` に `stdin=subprocess.DEVNULL` を渡す。**中断契約を所有する本 task で決着させること。**
    - **(同上) `shell.run_shell` の `stop_event` には現時点で producer が存在しない。** `watchdog_thread` (`service.py:701`) は立てず、SIGTERM/SIGINT ハンドラは `if daemon:` 配下のみ、`run_shell` に到達するのは**非 daemon 経路だけ**。**Task 17 は seam を用意しただけで、end-to-end では一度も駆動されていない。** 本 task で producer を配線し、**対話モードで実際に停止できることを E2E で確認**すること。
