...
| 2026-09-12 | v1.4 | 1 周目: codex (sol) Important 3 → 是正 C4 (transcript dir が `data/` 配下なら mkdir しない) / C5 (`.locks` sweep の `name` 非 str 隔離) / C6 (撤去した期限検査の docstring・設計書 127/157 行・foundation 設計書注記)。ローカル 3 本 (muse/ornith/qwen3.8、4 チャンク) 本番欠陥 0、pin 4 採用 / 却下 2 / 反証 2 (allowlist 子孫検出、401 分解)。fresh worktree フルスイート 3783 passed (失敗は `test_shell_interrupt.py` 9 件 = 既知 fd 枯渇)、残骸ゼロ、`logs/` 未生成 | `tmp/review-20260912-th/{codex-r1,verified-local}.md` | 5064c5c, (pin commit) |
| 2026-09-12 | v1.5 | 2 周目 (ユーザー裁定: codex のみ、是正差分 3413fe9..a22663f 限定) Critical 0 / Important 0 / Minor 2 → 是正 M1 (`.locks` sweep の name を `_PLUGIN_NAME_RE` で検証、空文字 pending が `.locks/.lock` を残す実測を codex が示し、ローカル 1 周目の却下を撤回) / M2 (C4 warning「fail closed」→ 起動継続の明示)。レビューサイクル終了 | `tmp/review-20260912-th/codex-r2.md` | 8f867d9 |
| 2026-09-12 | v1.6 | 最終フルスイート (856e80d): 3786 passed / 既知 9 failed、`git status` 不変だが**空 `logs/` が残留** (gitignore のため status guard の盲点)。犯人 = `test_bootstrap_improve_profile_mission_transcript_dir_is_writable` の teardown が親を残す → 祖先も片付け + session guard に `logs/` 残留検査 (`_logs_dir_leaked`)。教訓: 「残骸ゼロ」の判定は git status だけでなく gitignore 領域 (`logs/` `data/`) の存在も見る | 指揮者の独立再確認 | 6bd3d35 |
| 2026-09-12 | v1.7 | main へ ff-merge + push (f39bed5..38ee7dd、12 commit)。**記録の欠落**: 実装報告 `tmp/th/impl-report.md` と逆変異記録 `tmp/th/mut-th.json` は worktree 内 (`tmp/wt/th/tmp/th/`) に置いたため `git worktree remove` で消失。内容の要旨は本変更履歴 v1.2〜v1.6 と各 commit message に残る。レビュー記録 `tmp/review-20260912-th/` (codex-r1/r2、verified-local、材料) は本体側で残存 | 教訓: worktree 内の tmp/ 成果物は remove 前に本体へ退避する | 38ee7dd |
