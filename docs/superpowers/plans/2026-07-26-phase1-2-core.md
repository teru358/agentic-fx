...
- tick の仕事 (設計書 §5)。市場オープン中は次の順で実行:
  1. **mark-to-market snapshot**: balance (broker) + open ポジションの含み損益 (直近バー close で時価評価、バーが無いペアは entry 価格 = 含み 0 の保守近似) から equity を算出し `record_snapshot` — **gate の equity/hwm の鮮度はこれが供給する** (kill switch の「unrealized 込み」を Phase 1 でも成立させる)
  2. **unknown 解決 (reconcile)**: `*_unknown` の orders に `broker.reconcile` → 結果に応じて遷移 (paper の `not_found`: submit_unknown → rejected / cancel_unknown → cancelled / close_unknown → closing 再試行)。解決するまで新規発注は gate が停止 (設計書 §12)
  ...
  4. **指値予約の維持** (設計書 §5): 現在 equity で総リスク・レバレッジ上限を再評価し、**維持できない間は `pending_fill` を新しい順に自動取消** (executor.cancel_order(reason="reservation"))
  ...
  6. ペーパー約定処理: `pending_fill` に `check_limit_fill` → 約定なら protection_pending → open。`open` に `check_exit` → `close_order(reason="sl"|"tp")`。エントリーと同一 tick のバーは `entry_same_bar=True`。**バーは鮮度検証 (5 分以内) + 同一バー再処理防止 (pair 毎に処理済み bar.ts を記録)**
  7. 毎時 (前回から 60 分以上): `on_trade_mission`
- 市場クローズ中: `on_news_cycle` のみ (30 分毎)。**クローズ移行の瞬間** (前回 tick は open): trading モードなら未約定実指値を全取消 (learning は pending_fill を残す)
