...
**レビュー対応 修正ラウンド 3 (Important 相当の設計不整合の修正, 2026-07-26,
別コミット)**: 修正ラウンド 2 で `_retry_close` に追加した
`broker.close()` の except は、例外を「未実行 (quote 障害と同様)」とみなし
`CLOSING` のまま次 tick 再試行としていたが、これは論理的に不整合だった。
`quote_fn` の例外は broker に触れる前に起きるため未実行が確定するが、
`broker.close()` の例外 (タイムアウト・接続断) は **broker 側では成功して
いるかもしれない** — 黙って `CLOSING` のまま再試行すると、Phase 3 の MT5
実装で reconcile せず盲目的に再 close する**二重クローズ**のリスクになる
(同じメソッド内のすぐ下の `br2.status != "ok"` → `S.CLOSE_UNKNOWN` 分岐、
および executor 側の「broker 例外は一律 unknown に正規化する」設計とも
矛盾していた)。

`broker.close()` の except だけを `S.CLOSE_UNKNOWN` へ遷移させる (activity
+ notifier で記録) よう変更。`quote_fn` の except は現状どおり (状態を
変えず次 tick 再試行) を維持し、**例外の発生源 (broker に触れる前か後か)
で扱いが変わることがコード上明確になる**構造にした。

追加テスト 2 件
(`test_close_retry_broker_exception_marks_close_unknown_not_closing` /
`test_close_retry_quote_exception_stays_closing_for_next_tick_retry`)。
既存の `test_close_retry_broker_exception_skips_order_but_tick_continues`
はアサーションを `closing` → `close_unknown` に修正 (誤った中間挙動を検証
していたテストの是正)。`uv run pytest tests/core/test_scheduler.py -v` は
28 件全て PASS (既存 26 + 新規 2)。`uv run pytest` (全体) は 259 件 PASS
(修正ラウンド 3 着手前 257 + 新規 2、破壊なし)。詳細は
`.superpowers/sdd/2026-07-26-phase1-2-core/task-fix-codex-report.md` の
追記を参照。
